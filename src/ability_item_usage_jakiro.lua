...
local CanCast = {
	...
	utility.NCanCast,
	function(t)
		return AbilityExtensions:NormalCanCast(
			t,
			true,
			AbilityExtensions:HasScepter(npcBot) and DAMAGE_TYPE_PURE or DAMAGE_TYPE_MAGICAL,
			false,
			false
		)
	end
...
Consider[3] = function()
	...
	if
	 ...
	 then
		...
		if (WeakestCreep ~= nil) then
			if (CanCast[abilityNumber](WeakestCreep) and GetUnitToUnitDistance(npcBot, WeakestCreep) < CastRange + 75 * #allys) then
				return BOT_ACTION_DESIRE_LOW, WeakestCreep
			end
		end
	end
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (#enemys >= 1) then
			if (CanCast[abilityNumber](enemys[1])) then
...
Consider[5] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if HeroHealth <= GetDamage(WeakestEnemy) * (0.9 + #allys * 0.1) then
					local d = GetUnitToUnitDistance(npcBot, WeakestEnemy)
					if (d <= CastRange + Radius) then

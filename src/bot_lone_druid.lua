...
function MinionThink(hMinionUnit)
	if not hMinionUnit:IsNull() and hMinionUnit ~= nil then
		if string.find(hMinionUnit:GetUnitName(), "npc_dota_lone_druid_bear") then
			...
			if (hMinionUnit:IsUsingAbility() or hMinionUnit:IsChanneling() or not hMinionUnit:IsAlive()) then
				return
			end
			...
			if (RetreatDesire > 0) then
				...
				if bearState == "" and RetreatDesire == BOT_ACTION_DESIRE_HIGH then
					bearState = "retreat"
				end
				return
			end
...
function ConsiderReturn(hMinionUnit)
	if
		bearState == "retreat" or RetreatDesire > 0 or abilityFG:GetLevel() < 2 or not abilityRT:IsFullyCastable() or
			abilityRT:IsHidden()
	 then
...
function ConsiderSavageRoar(hMinionUnit)
	...
	if
		(npcBot:GetActiveMode() == BOT_MODE_ROAM or npcBot:GetActiveMode() == BOT_MODE_ATTACK or
			npcBot:GetActiveMode() == BOT_MODE_DEFEND_ALLY)
	 then
		local npcTarget = npcBot:GetTarget()
		if (npcTarget ~= nil and npcTarget:IsHero()) then
			if
				(CanCastSavageRoarOnTarget(npcTarget) and GetUnitToUnitDistance(npcTarget, hMinionUnit) < nRadius and
...
function ConsiderRetreat(hMinionUnit)
	...
	if
		not (mode == BOT_MODE_ATTACK or mode == BOT_MODE_DEFEND_ALLY) and
			(hMinionUnit:WasRecentlyDamagedByAnyHero(2.0) or hMinionUnit:WasRecentlyDamagedByTower(2.0))
	 then

...
function GetStickyNapalmCount(npcTarget)
	...
	if (modifier ~= nil) and modifier ~= -1 then
...
Consider[2] = function()
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy) and GetStickyNapalmCount(npcEnemy) >= 3) then
				return BOT_ACTION_DESIRE_HIGH, npcEnemy:GetExtrapolatedLocation(CastPoint)
			end
		end
	end
...
Consider[3] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (npcBot:HasModifier("modifier_batrider_firefly")) then
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (ManaPercentage > 0.6 or npcBot:GetMana() > ComboMana) then
			if (WeakestEnemy ~= nil) then
				if (CanCast[abilityNumber](WeakestEnemy) and GetStickyNapalmCount(WeakestEnemy) >= 3) then
		...
		if (npcEnemy ~= nil) then
			if
				(CanCast[abilityNumber](npcEnemy) and not enemyDisabled(npcEnemy) and
					GetUnitToUnitDistance(npcBot, npcEnemy) < CastRange + 75 * #allys and
					GetStickyNapalmCount(npcEnemy) >= 3)
			 then

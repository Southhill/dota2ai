...
Consider[5] = function()
	 ...
	 then
		...
		if #realEnemies ~= 0 then
			return BOT_ACTION_DESIRE_HIGH, AbilityExtensions:Max(
				realEnemies,
				function(t)
					return t:GetMaxHealth() * RemapValClamped(AbilityExtensions:GetHealthPercent(t), 0, 0.4, 2, 1)
				end
			)
		end
		if #enemies ~= 0 then
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		...
		if (StrongestCreep ~= nil and StrongestCreep:GetHealth() / StrongestCreep:GetMaxHealth() > 0.5) then
			return BOT_ACTION_DESIRE_MODERATE, StrongestCreep
		end
		if (StrongestCreepAlly ~= nil) then
...
Consider[7] = function()
	...
	-- If we're seriously retreating, see if we can land a stun on someone who's damaged us recently
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT) then
		if (#enemys == 0 or AbilitiesReal[1]:IsFullyCastable() and npcBot:DistanceFromFountain() >= 1000) then
			return BOT_ACTION_DESIRE_HIGH
		end
	end

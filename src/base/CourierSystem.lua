...
local function IsTargetedByUnit(courier)
    ...
    for _, id in pairs(GetTeamPlayers(GetOpposingTeam())) do
        if IsHeroAlive(id) then
            ...
            if info ~= nil then
                local dInfo = info[1]
                if dInfo ~= nil and GetUnitToLocationDistance(courier, dInfo.location) <= 700 and dInfo.time_since_seen <
                    0.5 then
                    return true
                end
            end
...
function CourierUtility.CourierUsageThink()
    ...
    if pIDInc < #pIDs + 1 and DotaTime() > -60 then
        if IsPlayerBot(pIDs[pIDInc]) == true then
            local currID = pIDs[pIDInc]
            if npcBot:GetPlayerID() == currID then
                if checkCourier == true and DotaTime() > calibrateTime + 5 then
    ...
    if npcBot.courierAssigned == true then
        ...
        if cState == COURIER_STATE_AT_BASE or cState == COURIER_STATE_IDLE or cState == COURIER_STATE_RETURNING_TO_BASE then
            ...
            if npcBot:IsAlive() and npcBot:GetCourierValue() > 0 then
            ...
            if not npcBot:IsAlive() and cState == COURIER_STATE_DELIVERING_ITEMS and npcBot:GetCourierValue() > 0 and
                DotaTime() > courierTime + 1.0 then
                npcBot:ActionImmediate_Courier(npcCourier, COURIER_ACTION_RETURN_STASH_ITEMS)
                npcCourier.latestUser = npcBot
                courierTime = DotaTime()
                -- print("Return Item");

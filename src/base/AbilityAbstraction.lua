...
for k, v in pairs(Func) do
    AbilityUtility[k] = function(self, ...)
        return v(...)
    end
end
...
-- 将欲望值裁剪到 0~1 之间
AbilityUtility.TrimDesire = function(self, desire)
    return Trim(desire, 0, 1)
end

-- 根据技能冷却时间计算技能重要性（冷却越长=越重要）
AbilityUtility.GetAbilityImportance = function(self, cooldown)
    return Trim(cooldown / 120, 0, 1)
end

-- 判断 Bot 是否处于打钱或推进模式
AbilityUtility.IsFarmingOrPushing = function(self, npcBot)
...
-- 判断 Bot 是否处于对线模式
AbilityUtility.IsLaning = function(self, npcBot)
    local mode = npcBot:GetActiveMode()
    return mode == BOT_MODE_LANING
end

-- 判断 Bot 是否处于攻击敌方模式
AbilityUtility.IsAttackingEnemies = function(self, npcBot)
    local mode = npcBot:GetActiveMode()
...
-- 判断 Bot 是否处于撤退模式
AbilityUtility.IsRetreating = function(self, npcBot)
    return npcBot:GetActiveMode() == BOT_MODE_RETREAT
end

-- 判断 Bot 是否不处于撤退模式
AbilityUtility.NotRetreating = function(self, npcBot)
    return npcBot:GetActiveMode() ~= BOT_MODE_RETREAT
end

-- 判断是否有足够蓝量使用攻击附带类技能
AbilityUtility.HasEnoughManaToUseAttackAttachedAbility = function(self, npcBot, ability)
...
AbilityUtility.ToggleFunctionToAction = function(self, npcBot, oldConsider, ability)
    return function()
...
AbilityUtility.ToggleFunctionToAutoCast = function(self, npcBot, oldConsider, ability)
    return function()
        local value, target, castType = oldConsider()
        if type(value) == "number" then
            return value, target, castType
...
-- 包装原技能考虑函数，防止对幻象使用技能
AbilityUtility.PreventAbilityAtIllusion = function(self, npcBot, oldConsiderFunction, ability)
    return function()
...
AbilityUtility.PreventEnemyTargetAbilityUsageAtAbilityBlock = function(self, npcBot, oldConsiderFunction, ability)
...
AbilityUtility.GetUsedAbilityInfo = function(self, ability, abilityInfoTable, considerTarget)
...
AbilityUtility.AddCooldownToChargeAbility = function(self, oldConsider, ability, abilityInfoTable, additionalCooldown)
    return function()
...
AbilityUtility.AutoModifyConsiderFunction = function(self, npcBot, considers, abilitiesReal)
...
function AbilityUtility:InitAbility(npcBot)
...
local keysBeforeAbilityInformation = AbilityUtility:Keys(AbilityUtility)

AbilityUtility.UndisjointableProjectiles = { "alchemist_berser_potion", "alchemist_unstable_concoction_throw",
...
AbilityUtility.targetTrackingStunAbilities = { "alchemist_berser_potion", "alchemist_unstable_concoction_throw",
...
AbilityUtility.targetNonTrackingStunAbilities = { "bane_fiends_grip", "beastmaster_primal_roar",
...
AbilityUtility.targetStunAbilities = AbilityUtility:Concat(AbilityUtility.targetTrackingStunAbilities,
    AbilityUtility.targetNonTrackingStunAbilities)

AbilityUtility.locationStunAbilities = { "axe_berserkers_call", "centaur_hoof_stomp", "dark_seer_vacuum",
    "dark_willow_cursed_crown",
...
AbilityUtility.targetTrackingDisableAbilities = { "gleipnir_eternal_chains", "naga_siren_ensnare", "riki_sleeping_dart",
    "viper_viper_strike" }

AbilityUtility.targetNonTrackingDisableAbilities = { "bloodseeker_rupture", "doom_bringer_doom",
    "ember_spirit_searing_chains",
    "grimstroke_ink_creature", "grimstroke_soul_chain", "item_sheepstick",
    "lion_vex", "shadow_demon_purge", "shadow_shaman_voodoo" }

AbilityUtility.targetDisableAbilities = AbilityUtility:Concat(AbilityUtility.targetNonTrackingStunAbilities,
    AbilityUtility.targetTrackingDisableAbilities)

AbilityUtility.locationDisableAbilities = { "dark_willow_bramble_maze", "death_prophet_silence",
    "disruptor_kinetic_field",
    "disruptor_static_storm", "drow_ranger_wave_of_silence", "elder_titan_echo_stomp",
    "invoker_deafening_wave", "treant_overgrowth" }

AbilityUtility.targetTrackingHeavyDamageAbilities = { "item_ethereal_blade", "lich_chain_frost", "lion_finger_of_death",
    "morphling_adaptive_strike_agi", "sniper_assassinate" }

AbilityUtility.targetNonTrackingHeavyDamageAbilities = { "antimage_mana_void", "item_dagon", "lina_laguna_blade",
    "pugna_life_drain",
    "tinker_laser", "zuus_lightning_bolt" }

AbilityUtility.targetHeavyDamageAbilities = AbilityUtility:Concat(AbilityUtility.targetTrackingHeavyDamageAbilities,
    AbilityUtility.targetNonTrackingHeavyDamageAbilities)

AbilityUtility.locationHeavyDamageAbilities = { "ancient_apparition_ice_blast", "antimage_mana_void",
...
AbilityUtility.heavyDamageAbilities = AbilityUtility:Concat(AbilityUtility.targetTrackingHeavyDamageAbilities,
    AbilityUtility.locationHeavyDamageAbilities)

AbilityUtility.dodgeWorthAbilities = AbilityUtility:Concat(AbilityUtility.targetStunAbilities,
    AbilityUtility.locationStunAbilities, AbilityUtility.heavyDamageAbilities)

AbilityUtility.invisibleModifiers = { "modifier_bounty_hunter_wind_walk", "modifier_clinkz_wind_walk",
...
AbilityUtility.phaseModifiers = { "modifier_bounty_hunter_wind_walk", "modifier_clinkz_wind_walk",
...
AbilityUtility.phaseUnits = { "npc_dota_brewmaster_fire_1", "npc_dota_brewmaster_fire_2", "npc_dota_brewmaster_fire_3",
...
AbilityUtility.unobstructedMovementModifiers = { "modifier_batrider_firefly", "modifier_broodmother_spin_web",
...
AbilityUtility.flyingModifiers = { "modifier_rattletrap_jetpack", "modifier_night_stalker_darkness",
    "modifier_winter_wyvern_arctic_burn_flight" }

AbilityUtility.flyingUnits = { "npc_dota_visage_familiar1", "npc_dota_visage_familiar2", "npc_dota_visage_familiar3",
    "npc_dota_flying_courier", "npc_dota_beastmaster_hawk" }

AbilityUtility.positiveForceMovementModifiers = { "modifier_faceless_void_time_walk", "modifier_huskar_life_break_charge",
...
AbilityUtility.timeSensitivePositiveModifiers = { "modifier_item_black_king_bar",
...
AbilityUtility.basicDispellablePositiveModifiers = { "modifier_omniknight_guardian_angle",
...
AbilityUtility.basicDispellWorthPositiveModifiers = { "modifier_omniknight_guardian_angle",
    "modifier_ember_spirit_flame_guard",
    "modifier_legion_commander_press_the_attack", "modifier_windrunner_windrun",
    "modifier_lich_frost_shield", "modifier_oracle_purifying_flames",
    "modifier_ogre_magi_bloodlust", "modifier_treant_living_armor",
...
AbilityUtility.basicDispellWorthNegativeModifiers = { "modifier_abaddon_frostmourne_debuff_bonus" }

AbilityUtility.basicDispellableNegativeModifiers = { "modifier_abaddon_frostmourne_debuff",
    "modifier_abaddon_frostmourne_debuff_bonus" }

AbilityUtility.unbreakableChannelAbilities = { "puck_phase_shift", "pangolier_gyroshell", "lone_druid_true_form",
    "phoenix_supernova",
    "lycan_shapeshift" }

AbilityUtility.nonIllusionModifiers = {}

AbilityUtility.valubleNeutrals = { "npc_dota_neutral_alpha_wolf", "npc_dota_neutral_centaur_khan",
...
AbilityUtility.valubleAncientNeutrals = { "npc_dota_neutral_black_dragon", "npc_dota_neutral_rock_golem",
    "npc_dota_neutral_big_thunder_lizard" }

AbilityUtility.hypnosisModifiers = { "modifier_lich_sinister_gaze", "modifier_void_spirit_aether_remnant_pull",
    "modifier_keeper_of_the_light_will_o_wisp" }

AbilityUtility.fearModifiers = { "modifier_dark_willow_debuff_fear", "modifier_lone_druid_savage_roar",
    "modifier_shadow_fiend_requiem_fear", "modifier_terrorblade_fear" }

AbilityUtility.hexModifiers = { "modifier_lion_voodoo", "modifier_shadow_shaman_voodoo", "modifier_sheepstick_debuff",
    "modifier_item_princes_knife_hex", "modifier_hexxed" -- dazzle_poison_touch lv 25
}

AbilityUtility.silenceModifiers = { "modifier_abaddon_frostmourne_debuff_bonus", "modifier_silence",
...
AbilityUtility.timedSilenceModifiers = { "modifier_abaddon_frostmourne_debuff_bonus", "modifier_silence",
...
AbilityUtility.magicImmuneModifiers = { "modifier_item_black_king_bar", "modifier_life_stealer_rage",
...
AbilityUtility.muteModifiers = { "modifier_tusk_snowball", "modifier_doom_bringer_doom",
    "modifier_disruptor_static_storm_mute" }

AbilityUtility.breakModifiers = { -- "modifier_doom_bringer_doom",--only breaks with scepter
...
AbilityUtility.noTrueSightRootAbilityAssociation = {
...
AbilityUtility.conditionalTrueSightRootAbilityAssociation = {
...
AbilityUtility.permanentTrueSightRootAbilityAssociation = {
...
AbilityUtility.rootAbilityAssociation = AbilityUtility:Concat(AbilityUtility.noTrueSightRootAbilityAssociation,
    AbilityUtility.conditionalTrueSightRootAbilityAssociation,
    AbilityUtility.permanentTrueSightRootAbilityAssociation)

local keysAfterAbilityInformation = AbilityUtility:Keys(AbilityUtility)
...
abilityInformationKeys:ForEach(function(t)
    setmetatable(AbilityUtility[t], magicTable)
end)
...
abilityInformationKeys:ForEach(function(t)
    local a, b = ExtendAssociation(AbilityUtility[t])
    local k = t:sub(1, #t - #"AbilityAssociation")
    AbilityUtility[k .. "Abilities"] = a
    AbilityUtility[k .. "Modifiers"] = b
end)

-- unit function

function AbilityUtility:IsRoshan(npcTarget)
    return npcTarget ~= nil and npcTarget:IsAlive() and string.find(npcTarget:GetUnitName(), "roshan")
end

function AbilityUtility:IsHero(t)
    return t:IsHero()
end

function AbilityUtility:IsTempestDouble(npc)
    return npc:HasModifier("modifier_arc_warden_tempest_double")
end

function AbilityUtility:IsLoneDruidBear(npc)
    return string.match(npc:GetUnitName(), "npc_dota_lone_druid_bear")
end

function AbilityUtility:IsVisageFamiliar(npc)
    return string.match(npc:GetUnitName(), "npc_dota_visage_familiar")
end

function AbilityUtility:IsBrewmasterPrimalSplit(npc)
    local unitName = npc:GetUnitName()
    return string.match(unitName, "npc_dota_brewmaster_")
end

AbilityUtility.GetIncomingDodgeWorthProjectiles = function(self, npc)
...
AbilityUtility.GetTargetHealAmplifyPercent = function(self, npc)
...
AbilityUtility.IsChannelingItem = function(self, npc)
...
AbilityUtility.IsChannelingAbility = function(self, npc)
    return npc:IsChanneling() and not self:IsChannelingItem(npc)
end

function AbilityUtility:IsChannelingBreakWorthAbility(npc)
...
AbilityUtility.RadiantPlayerId = GetTeamPlayers(TEAM_RADIANT)
AbilityUtility.DirePlayerId = GetTeamPlayers(TEAM_DIRE)

AbilityUtility.GetTeamPlayers = function(self, team)
...
AbilityUtility.GetEnemyTeamMemberNames = function(self, npcBot)
...
AbilityUtility.enemyVisibleIllusionModifiers = { "modifier_illusion", -- "modifier_phantom_lancer_doppelwalk_illusion",
...
AbilityUtility.MustBeIllusion = function(self, npcBot, target)
...
AbilityUtility.MayNotBeIllusion = function(self, npcBot, target)
    return not self:MustBeIllusion(npcBot, target)
end

function AbilityUtility:IsOnSameTeam(a, b)
    return a:GetTeam() == b:GetTeam()
end

function AbilityUtility:IsNonIllusionHero(npcBot, target)
    return self:MayNotBeIllusion(npcBot, target) and self:IsHero(target)
end

function AbilityUtility:HasNonIllusionModifier(npc)
    return self:HasAnyModifier(npc, self.nonIllusionModifiers)
end

function AbilityUtility:CanIllusionUseAbility(npc)
...
AbilityUtility.DetectIllusion = function(self, npcBot)
...
AbilityUtility.GetNearbyHeroes = function(self, npcBot, range, getEnemy, botModeMask)
    range = range or 1200
...
AbilityUtility.GetNearbyNonIllusionHeroes = function(self, npcBot, range, getEnemy, botModeMask)
    range = range or 1200
    if getEnemy == nil then
        getEnemy = true
    end
...
function AbilityUtility:AttackOnceDamage(npcBot, target)
    return target:GetActualIncomingDamage(npcBot:GetAttackDamage() - npcBot:GetBaseDamageVariance() / 2,
        DAMAGE_TYPE_PHYSICAL)
end

function AbilityUtility:GetNearbyAttackableCreeps(npcBot, range, getEnemy)
    if getEnemy == nil then
        getEnemy = true
    end
...
AbilityUtility.GetNearbyAllUnits = function(self, npcBot, range)
...
AbilityUtility.GetMovementSpeedPercent = function(self, npc)
    return npc:GetCurrentMovementSpeed() / npc:GetBaseMovementSpeed()
end
AbilityUtility.CanHardlyMove = function(self, npc)
    return npc:IsStunned() or npc:IsRooted() or npc:GetCurrentMovementSpeed() <= 150
end

AbilityUtility.GetModifierRemainingDuration = function(self, npc, modifierName)
...
AbilityUtility.imprisonmentModifier = { "modifier_item_cyclone", "modifier_item_wind_waker",
...
AbilityUtility.GetImprisonmentRemainingDuration = function(self, npc)
...
function AbilityUtility:GetMagicImmuneRemainingDuration(npc)
...
function AbilityUtility:GetSilenceRemainingDuration(npc)
...
function AbilityUtility:GetStunRemainingDuration(npc)
    return self:IsStunned(npc) and 1 or 0
end

AbilityUtility.GetEnemyHeroNumber = function(self, npcBot, enemies)
    return #self:GetEnemyHeroUnique(npcBot, enemies)
end

function AbilityUtility:HasPhasedMovement(npc)
    return self:HasAnyModifier(npc, self.phaseModifiers) or self:Contains(self.phaseUnits, npc:GetUnitName())
end

function AbilityUtility:HasUnobstructedMovement(npc)
...
AbilityUtility.GetAvailableItem = function(self, npc, itemName)
    for i = 0, 5 do
...
AbilityUtility.GetAncientLocation = function(self, npc)
...
AbilityUtility.GetDistanceFromAncient = function(self, npc)
    local fountain = self:GetAncientLocation(npc)
    return GetUnitToLocationDistance(npc, fountain)
end

AbilityUtility.TryUseTp = function(self, npc)
...
AbilityUtility.GetAvailableBlink = function(self, npc)
...
function AbilityUtility:GetAvailableTravelBoots(npc)
...
AbilityUtility.GetEmptyInventorySlots = function(self, npc)
    local g = 0
    for i = 0, 5 do
...
AbilityUtility.GetEmptyItemSlots = function(self, npc)
    local g = 0
    for i = 0, 8 do
        if npc:GetItemInSlot(i) == nil then
            g = g + 1
...
AbilityUtility.GetEmptyBackpackSlots = function(self, npc)
    local g = 0
    for i = 7, 9 do
        if npc:GetItemInSlot(i) == nil then
            g = g + 1
...
AbilityUtility.SwapItemToBackpack = function(self, npc, itemIndex)
    for i = 6, 8 do
        if npc:GetItemInSlot(i) == nil then
...
AbilityUtility.GetCarriedItems = function(self, npc)
    local g = NewTable()
    for i = 0, 8 do
        local item = npc:GetItemInSlot(i)
        if item ~= nil then
...
AbilityUtility.GetInventoryItems = function(self, npc)
    local g = NewTable()
    for i = 0, 5 do
        local item = npc:GetItemInSlot(i)
        if item ~= nil then
...
AbilityUtility.GetInventoryItemNames = function(self, npc)
    local g = NewTable()
    for i = 0, 5 do
        local item = npc:GetItemInSlot(i)
        if item ~= nil then
...
AbilityUtility.GetStashItems = function(self, npc)
    local g = NewTable()
    for i = 9, 14 do
        local item = npc:GetItemInSlot(i)
        if item ~= nil then
...
function AbilityUtility:GetCourierItems(courier)
    local g = NewTable()
    for i = 0, 8 do
...
function AbilityUtility:GetMyCourier(npcBot)
...
function AbilityUtility:FindCourier(npcBot)
    for i = 0, 4 do
...
AbilityUtility.GetAllBoughtItems = function(self, npcBot)
    local g = NewTable()
    for i = 0, 15 do
...
AbilityUtility.IsBoots = function(self, item)
...
AbilityUtility.SwapCheapestItemToBackpack = function(self, npc)
...
AbilityUtility.SuitableForSilence = function(self, npc, target)
    return self:MayNotBeIllusion(npc, target) and not target:IsMagicImmune() and not self:IsInvulnerable(target)
end

AbilityUtility.GetHeroFullName = function(self, s)
    return "npc_dota_hero_" .. s
end
AbilityUtility.GetHeroShortName = function(self, s)
    return string.sub(s, 12)
end

AbilityUtility.IsMeleeHero = function(self, npc)
...
function AbilityUtility:HasAnyModifier(npc, modifierGroup)
    return self:First(modifierGroup, function(t)
        return npc:HasModifier(t)
    end)
end

AbilityUtility.AttackPassiveAbilities = { "doom_bringer_infernal_blade", "drow_ranger_frost_arrows", "clinkz_fire_arrows",
    "viper_poison_attack", "obsidian_destroyer_arcane_orb" }
AbilityUtility.OtherIgnoreAbilityBlockAbilities = { "batrider_flaming_lasso", "gyrocopter_homing_missile",
    "axe_culling_blade" }
AbilityUtility.IgnoreAbilityBlockAbilities = { "dark_seer_ion_shell", "grimstroke_soulbind", "rubick_spell_steal",
...
AbilityUtility.IgnoreAbilityBlock = function(self, ability)
    local abilityName = ability:GetName()
...
AbilityUtility.AbilityRetargetModifiers = { "modifier_antimage_counterspell", "modifier_item_lotus_orb_active",
    "modifier_nyx_assassin_spiked_carapace" -- "modifier_item_blade_mail",
}
AbilityUtility.HasAbilityRetargetModifier = function(self, npc)
    return self:HasAnyModifier(npc, self.AbilityRetargetModifiers)
end

AbilityUtility.CanMove = function(self, npc)
    return not npc:IsStunned() and not npc:IsRooted() and not self:IsNightmared(npc) and not self:IsTaunted(npc)
end

function AbilityUtility:CannotMove(npc)
...
AbilityUtility.IsSeverelyDisabledOrSlowed = function(self, npc)
    return self:IsSeverelyDisabled(npc) or self:GetMovementSpeedPercent(npc) <= 0.35
end

AbilityUtility.HasSeverelyDisableProjectiles = function(self, npc)
...
AbilityUtility.IsOrGoingToBeSeverelyDisabled = function(self, npc)
    return self:IsSeverelyDisabled(npc) or self:HasSeverelyDisableProjectiles(npc)
end

AbilityUtility.EtherealModifiers = { "modifier_ghost_state", "modifier_item_ethereal_blade_ethereal",
    "modifier_necrolyte_death_seeker", "modifier_necrolyte_sadist_active", "modifier_pugna_decrepify" }
AbilityUtility.IsEthereal = function(self, npc)
    return self:HasAnyModifier(npc, self.EtherealModifiers)
end

function AbilityUtility:NotBlasted(npc)
    return not npc:HasModifier("modifier_ice_blast")
end

AbilityUtility.CannotBeTargetted = function(self, npc)
    return self:HasAnyModifier(npc, self.CannotBeTargettedModifiers)
end

AbilityUtility.CanBeTargettedFunction = function(npc)
    return not AbilityUtility:CanBeTargetted(npc)
end

AbilityUtility.CannotBeAttacked = function(self, npc)
    return self:IsEthereal(npc) or self:IsInvulnerable(npc) or self:CannotBeTargetted(npc)
end

AbilityUtility.IsInvulnerable = function(self, npc)
    return npc:IsInvulnerable() or self:Any(self.IgnoreDamageModifiers, function(t)
        return npc:HasModifier(t)
    end)
end

AbilityUtility.MayNotBeSeen = function(self, npc)
...
AbilityUtility.ShouldNotBeAttacked = function(self, npc)
...
AbilityUtility.PhysicalCanCastFunction = function(npc)
    return not AbilityUtility:IsInvulnerable(npc) and not AbilityUtility:ShouldNotBeAttacked(npc) and
        not npc:IsMagicImmune()
end

AbilityUtility.IsPhysicalOutputDisabled = function(self, npc)
    return npc:IsDisarmed() or npc:IsBlind() and not npc:GetAvailableItem("item_monkey_king_bar") or
        self:IsEthereal(npc)
end

AbilityUtility.GetHealthPercent = function(self, npc)
    return npc:GetHealth() / npc:GetMaxHealth()
end

function AbilityUtility:GetPhysicalHealth(t)
    return t:GetHealth() * (1 + 0.06 * t:GetArmor()) / (1 - t:GetEvasion())
end

function AbilityUtility:GetBuildingPhysicalHealth(t)
...
AbilityUtility.GetManaPercent = function(self, npc)
    return npc:GetMana() / npc:GetMaxMana()
end

AbilityUtility.GetHealthDeficit = function(self, npc)
    return npc:GetMaxHealth() - npc:GetHealth()
end

function AbilityUtility:GetManaDeficit(npc)
    return npc:GetMaxMana() - npc:GetMana()
end

function AbilityUtility:GetTargetIfGood(npc)
...
function AbilityUtility:IndexOfBasicDispellablePositiveModifier(npc)
...
AbilityUtility.MidLaneTowers = { TOWER_MID_1, TOWER_MID_2, TOWER_MID_3 }
AbilityUtility.BotLaneTowers = { TOWER_BOT_1, TOWER_BOT_2, TOWER_BOT_3 }
AbilityUtility.TopLaneTowers = { TOWER_TOP_1, TOWER_TOP_2, TOWER_TOP_3 }

function AbilityUtility:GetLaningTower(npc)
...
AbilityUtility.DebugTable = function(self, tb)
    local msg = "{ "
    local DebugRec
...
AbilityUtility.DebugLongTable = function(self, tb)
...
AbilityUtility.DebugArray = function(self, tb)
...
AbilityUtility.PrintAbilities = function(self, npcBot)
    local abilityNames = "{\n"
    for i = 0, 23 do
        local ability = npcBot:GetAbilityInSlot(i)
...
function AbilityUtility:NormalCanCast(target, isPureDamageWithoutDisable, damageType, pierceMagicImmune, targetMustBeSeen,
                                      mustBeTargettable)
...
AbilityUtility.NormalCanCastFunction = function(target)
    return AbilityUtility:NormalCanCast(target)
end

function AbilityUtility:SpellCanCast(target, pierceMagicImmune, targetMustBeSeen, mustBeTargettable)
...
AbilityUtility.SpellCanCastFunction = function(target)
    return AbilityUtility:SpellCanCast(target)
end

function AbilityUtility:AllyCanCast(target, pierceMagicImmune)
    if pierceMagicImmune == nil then
        pierceMagicImmune = true
    end
...
AbilityUtility.AllyCanCastFunction = function(target)
    return AbilityUtility:AllyCanCast(target)
end

function AbilityUtility:NeutralCanCast(target)
end

function AbilityUtility:EnemyAllyCanCast(target, isPureDamageWithoutDisable, damageType, pierceMagicImmune,
                                         targetMustBeSeen)
...
AbilityUtility.SpecialBonusAttributes = "special_bonus_attributes"
AbilityUtility.TalentNamePrefix = "special_bonus_"
AbilityUtility.IncorrectAbilityName = "incorrect_name"

AbilityUtility.IsTalent = function(self, ability)
    if ability == nil then
        return false
    end
...
AbilityUtility.GetAbilities = function(self, npcBot)
    local g = NewTable()
    for i = 0, 25 do
        local ability = npcBot:GetAbilityInSlot(i)
        if ability ~= nil and ability:GetName() ~= "generic_hidden" then
...
AbilityUtility.GetPointToLineDistance = function(self, point, line)
    local up = math.abs(line.a * point.x + line.b * point.y + line.c)
    local down = math.sqrt(line.a * line.a + line.b * line.b)
    return up / down
end

AbilityUtility.GetPointToPointDistance = function(self, a, b)
...
AbilityUtility.GetPointFromLineByDistance = function(self, startPoint, endPoint, distance)
...
AbilityUtility.GetCos = function(self, b, c, a)
    return (b ^ 2 + c * 2 - a * 2) / 2 / b / c
end
AbilityUtility.GetLocationToLocationDistance = function(self, a, b)
    return ((a.x - b.x) ^ 2 + (a.y - b.y) ^ 2) ^ 0.5
end

function AbilityUtility:GetDegree(loc1, loc2)
...
--    return g
-- end
AbilityUtility.FindAOELocationAtSingleTarget = function(self, npcBot, target, radius, castRange, castPoint)
    radius = radius - 80
...
AbilityUtility.PreventHealAtHealSuppressTarget = function(self, npcBot, oldConsiderFunction, ability)
    return function()
        local desire, target, targetTypeString = oldConsiderFunction()
...
AbilityUtility.PURCHASE_ITEM_OUT_OF_STOCK = 82
AbilityUtility.PURCHASE_ITEM_INVALID_ITEM_NAME = 33
AbilityUtility.PURCHASE_ITEM_DISALLOWED_ITEM = 78
AbilityUtility.PURCHASE_ITEM_INSUFFICIENT_GOLD = 63
AbilityUtility.PURCHASE_ITEM_NOT_AT_SECRET_SHOP = 62
AbilityUtility.PURCHASE_ITEM_NOT_AT_HOME_SHOP = 67
AbilityUtility.PURCHASE_ITEM_SUCCESS = -1
...
AbilityUtility.IgnoreDamageModifiers = { "modifier_abaddon_borrowed_time", "modifier_item_aeon_disk_buff",
    "modifier_winter_wyvern_winters_curse", "modifier_winter_wyvern_winters_curse_aura",
    "modifier_skeleton_king_reincarnation_scepter_active" }

AbilityUtility.CannotKillModifiers = { "modifier_dazzle_shadow_grave", "modifier_troll_warlord_battle_trance" }

AbilityUtility.CannotBeTargettedModifiers = { "modifier_slark_shadow_dance", "modifier_item_book_of_shadows",
    "modifier_dark_willow_shadow_realm_buff" }

AbilityUtility.IgnorePhysicalDamageModifiers = { "modifier_winter_wyvern_cold_embrace" }
AbilityUtility.IgnoreMagicalDamageModifiers = { "modifier_oracle_fates_edict" }

AbilityUtility.LastForAtLeastSeconds = function(self, predicate, time, infoTable)
...
AbilityUtility.GoodIllusionHero = { "antimage", "spectre", "terrorblade", "naga_siren" }
AbilityUtility.ModerateIllusionHero = { "abaddon", "axe", "chaos_knight", "arc_warden", "juggernaut", "luna", "medusa",
    "morphling",
    "phantom_lancer", "sniper", "wraith_king", "phantom_assassin" }
AbilityUtility.GetIllusionBattlePower = function(self, npc)
...
AbilityUtility.GetNetWorth = function(self, npc, isEnemy)
    if isEnemy then
...
function AbilityUtility:GetBattlePower(npc)
    local power = 0
    local name = npc:GetUnitName()
...
AbilityUtility.GetHeroGroupBattlePower = function(self, npcBot, heroes, isEnemy)
    local function A(tb)
...
AbilityUtility.Outnumber = function(self, npcBot, friends, enemies)
    return self:GetHeroGroupBattlePower(npcBot, friends, false) >= self:GetHeroGroupBattlePower(npcBot, enemies, true) *
        1.8
end

AbilityUtility.CannotBeKilledNormally = function(self, target)
...
AbilityUtility.HasScepter = function(self, npc)
...
function AbilityUtility:RecordAbility(npc, index, target, castType, abilities)
...
AbilityUtility.GetFrameNumber = function(self) return Timer.GetFrameNumber() end
AbilityUtility.EveryManyFrames = function(self, ...) return Timer.EveryManyFrames(...) end
AbilityUtility.EveryManySeconds = function(self, ...) return Timer.EveryManySeconds(...) end
AbilityUtility.SingleForTeam = function(self, ...) return Timer.SingleForTeam(...) end
AbilityUtility.SingleForAllBots = function(self, ...) return Timer.SingleForAllBots(...) end
AbilityUtility.CalledOnThisFrame = function(self, ...) return Timer.CalledOnThisFrame(...) end
AbilityUtility.TickFromDota = function(self) Timer.TickFromDota() end
AbilityUtility.RegisterSlowFunction = function(self, ...) return Timer.RegisterSlowFunction(...) end
AbilityUtility.ResumeUntilReturn = function(self, ...) return Timer.ResumeUntilReturn(...) end
AbilityUtility.StartCoroutine = function(self, ...) return Timer.StartCoroutine(...) end
AbilityUtility.WaitForSeconds = function(self, ...) return Timer.WaitForSeconds(...) end
AbilityUtility.StopCoroutine = function(self, ...) Timer.StopCoroutine(...) end
    ...
    elseif type(oldIndex) == "function" then
        ...
        mt.__index = function(ability, key)
            ...
            if oldResult[1] == nil then
                ...
                return AbilityUtility.Unpack(oldResult)
            end
        end
        ...
        if oldIndex == mt then
            ...
            mt.__index = function(tbl, key)
                ...
                if newResult[1] == nil then
                    ...
                    return AbilityUtility.Unpack(newResult)
                end
            end
        else
...
function AbilityUtility:pcall(func, ...)
    local result = { func(...) }
    if result[1] then
...
-- M.debug = true
function AbilityUtility:DebugPause()
    if self.debug then
        DebugPause()
    end
end

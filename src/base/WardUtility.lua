...
function X.GetAggressiveSpot()
	...
	local AggressiveRadiant = { RADIANT_AGGRESSIVETOP, RADIANT_AGGRESSIVEMID1, RADIANT_AGGRESSIVEMID2,
	...
	for i, id in pairs(TeamPlayers) do
		if not IsPlayerBot(id) then
			...
			if member ~= nil and member:IsAlive() and GetUnitToUnitDistance(bot, member) <= 1000 then
				...
				if GetUnitToLocationDistance(bot, ping.location) <= 600 and GameTime() - ping.time < 5 and Wslot == -1 then
					return true, member
				end
			end
		end

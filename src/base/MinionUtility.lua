...
-- 各野怪技能变量（在 MinionThink 中赋值，在 ConsiderXXX 中读取）
local abilityWS = nil;
local abilityTC = nil;
local abilitySW = nil;
local abilityMB = nil;
...
function MinionThink(hMinionUnit)
    ...
    if not hMinionUnit:IsNull() and hMinionUnit ~= nil and hMinionUnit:IsAlive() then
        if hMinionUnit:IsIllusion() then
            if (hMinionUnit:IsUsingAbility() or hMinionUnit:IsChanneling()) then
                return
            end
...
function ConsiderAttacking(hMinionUnit)
    if hMinionUnit:DistanceFromFountain() == 0 and hMinionUnit:GetHealth() / hMinionUnit:GetMaxHealth() < 0.9 then
...
function ConsiderMove(hMinionUnit)
    ...
    if target == nil or (target ~= nil and not CanBeAttacked(target)) or
        (target ~= nil and GetUnitToUnitDistance(target, npcBot) > ProxRange and GetUnitToUnitDistance(target, npcBot) >
            300) then
        -- return BOT_ACTION_DESIRE_MODERATE, npcBot:GetLocation();
        return BOT_ACTION_DESIRE_MODERATE,
            npcBot:GetXUnitsTowardsLocation(GetAncient(GetOpposingTeam()):GetLocation(), 250);
    end
...
function ConsiderRetreat(hMinionUnit)
    ...
    if (hMinionUnit:DistanceFromFountain() == 0 and hMinionUnit:GetHealth() / hMinionUnit:GetMaxHealth() < 0.9) or
        hMinionUnit:GetHealth() / hMinionUnit:GetMaxHealth() < 0.25 then
        local location = DB;
...
function ConsiderThunderClap(hMinionUnit)
    ...
    if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
        ...
        for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
            if (npcBot:WasRecentlyDamagedByHero(npcEnemy, 1.0) and CanCastOnNonMagicImmuneTarget(npcEnemy)) then
    ...
    if (npcBot:GetActiveMode() == BOT_MODE_ROAM or npcBot:GetActiveMode() == BOT_MODE_TEAM_ROAM or
            npcBot:GetActiveMode() == BOT_MODE_GANK or npcBot:GetActiveMode() == BOT_MODE_ATTACK or npcBot:GetActiveMode() ==
            BOT_MODE_DEFEND_ALLY) then
        local npcTarget = npcBot:GetTarget();
        if (npcTarget ~= nil and npcTarget:IsHero()) then
            if (CanCastOnNonMagicImmuneTarget(npcTarget) and GetUnitToUnitDistance(hMinionUnit, npcTarget) < nRadius) then
                return BOT_ACTION_DESIRE_HIGH;
            end
        end
    end
...
function ConsiderShockWave(hMinionUnit)
    -- Make sure it's castable
    if (abilitySW:IsNull() or not abilitySW:IsFullyCastable()) then
        return BOT_ACTION_DESIRE_NONE, 0;
    end
    ...
    if (npcTargetToKill ~= nil and npcTargetToKill:IsHero() and CanCastOnNonMagicImmuneTarget(npcTargetToKill)) then
        if (npcTargetToKill:GetActualIncomingDamage(nDamage, DAMAGE_TYPE_MAGICAL) > npcTargetToKill:GetHealth() and
                GetUnitToUnitDistance(npcTargetToKill, hMinionUnit) < nCastRange) then
    ...
    if (npcBot:GetActiveMode() == BOT_MODE_ROAM or npcBot:GetActiveMode() == BOT_MODE_TEAM_ROAM or
        ...
        if (npcTarget ~= nil and npcTarget:IsHero() and GetUnitToUnitDistance(npcTarget, hMinionUnit) < nCastRange and
                CanCastOnNonMagicImmuneTarget(npcTarget)) then
...
function ConsiderManaBurn(hMinionUnit)
    ...
    -- If a mode has set a target, and we can kill them, do it
    local npcTarget = npcBot:GetTarget();
    if (npcTarget ~= nil and npcTarget:IsHero() and CanCastOnNonMagicImmuneTarget(npcTarget)) then
        if (npcTarget:GetActualIncomingDamage(nDamage, DAMAGE_TYPE_MAGICAL) > npcTarget:GetHealth() and
                GetUnitToUnitDistance(npcTarget, hMinionUnit) < (nCastRange + 200)) then
    ...
    if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
        local tableNearbyEnemyHeroes = hMinionUnit:GetNearbyHeroes(nCastRange, true, BOT_MODE_NONE);
        for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
            if (npcBot:WasRecentlyDamagedByHero(npcEnemy, 2.0) and CanCastOnNonMagicImmuneTarget(npcEnemy)) then
    ...
    if (npcBot:GetActiveMode() == BOT_MODE_ROAM or npcBot:GetActiveMode() == BOT_MODE_TEAM_ROAM or
            ...
            BOT_MODE_DEFEND_ALLY) then
        local npcTarget = npcBot:GetTarget();
        if (npcTarget ~= nil and npcTarget:IsHero() and CanCastOnNonMagicImmuneTarget(npcTarget) and
                GetUnitToUnitDistance(npcTarget, hMinionUnit) < nCastRange + 200) then
...
function ConsiderBoulder(hMinionUnit)
    ...
    if (npcTarget ~= nil and npcTarget:IsHero() and CanCastOnNonMagicImmuneTarget(npcTarget)) then
        if (npcTarget:GetActualIncomingDamage(nDamage, DAMAGE_TYPE_MAGICAL) > npcTarget:GetHealth() and
                GetUnitToUnitDistance(npcTarget, hMinionUnit) < nCastRange) then
            return BOT_ACTION_DESIRE_HIGH, npcTarget;
        end
    end
...
function ConsiderFrostArmor(hMinionUnit)
    ...
    for _, myFriend in pairs(tableNearbyFriendlyHeroes) do
        if (not myFriend:HasModifier("modifier_ogre_magi_frost_armor")) then

...
Consider[2] = function()
	...
	local allys = utility.GetNearbyVisibleHeroes(npcBot, 1200, false, BOT_MODE_NONE)
	local enemys = utility.GetNearbyVisibleHeroes(npcBot, CastRange + 300, true, BOT_MODE_NONE)
	local WeakestEnemy, HeroHealth =
		utility.GetWeakestUnit(
		AbilityExtensions:Filter(
			enemys,
			function(t)
				return not AbilityExtensions:HasAbilityRetargetModifier(t)
			end
		)
	)
	local creeps = npcBot:GetNearbyCreeps(CastRange + 300, true)
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if
					(HeroHealth <= WeakestEnemy:GetActualIncomingDamage(Damage, DAMAGE_TYPE_ALL) or
...
Consider[3] = function()
	...
	if (#tableNearbyAttackingAlliedHeroes >= 2) then
		...
		for _, npcEnemy in pairs(enemys) do
			...
			if (CanCast[abilityNumber](npcEnemy) and not enemyDisabled(npcEnemy) and allys2 ~= nil and #allys2 == 0) then
				local Damage2 = npcEnemy:GetEstimatedDamageToTarget(false, npcBot, 3.0, DAMAGE_TYPE_ALL)
				if (Damage2 > nMostDangerousDamage) then
					nMostDangerousDamage = Damage2
					npcMostDangerousEnemy = npcEnemy
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		for _, npcEnemy in pairs(enemys) do
			local allys2 = npcEnemy:GetNearbyHeroes(600, true, BOT_MODE_NONE)
			if (npcBot:WasRecentlyDamagedByHero(npcEnemy, 2.0) and allys2 ~= nil and #allys2 == 0) then
...
Consider[4] = function()
	...
	local allys = utility.GetNearbyVisibleHeroes(npcBot, 1200, false, BOT_MODE_NONE)
	local enemys = utility.GetNearbyVisibleHeroes(npcBot, CastRange + 300, true, BOT_MODE_NONE)
	local enemys2 = utility.GetNearbyVisibleHeroes(npcBot, CastRange - 300, true, BOT_MODE_NONE)
	local WeakestEnemy, HeroHealth = utility.GetWeakestUnit(enemys)
	local creeps = npcBot:GetNearbyCreeps(CastRange + 300, true)
	local WeakestCreep, CreepHealth = utility.GetWeakestUnit(creeps)
	--------------------------------------
	...
	for _, npcEnemy in pairs(enemys) do
		if
			(npcEnemy:IsChanneling() and CanCast[abilityNumber](npcEnemy) and
				not AbilityExtensions:HasAbilityRetargetModifier(npcEnemy))
		 then
			return BOT_ACTION_DESIRE_HIGH, npcEnemy
		end
	end
	...
	if
		AbilityExtensions:IsRetreating(npcBot) and #enemys == 1 and
			not AbilityExtensions:HasAbilityRetargetModifier(enemys[1])
	 then
	...
	local disabledAllies =
		AbilityExtensions:Filter(
		allys,
		function(t)
			return AbilityExtensions:IsSeverelyDisabled(t) and not t:IsChanneling()
		end
	)
	disabledAllies =

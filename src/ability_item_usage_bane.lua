...
Consider[2] = function()
    ...
    local allys = utility.GetNearbyVisibleHeroes(npcBot, 1200, false, BOT_MODE_NONE)
    local enemys = utility.GetNearbyVisibleHeroes(npcBot, CastRange + 300, true, BOT_MODE_NONE)
    local WeakestEnemy, HeroHealth = utility.GetWeakestUnit(AbilityExtensions:Filter(enemys, function(t)
...
Consider[4] = function()
    ...
    local disabledAllies = AbilityExtensions:Filter(allys, function(t)
        return AbilityExtensions:IsSeverelyDisabled(t) and not t:IsChanneling()
    end)
    disabledAllies = AbilityExtensions:SortByMinFirst(disabledAllies, function(t)
        return t:GetHealth()
    end)

...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_ROSHAN) then
		...
		if (npcTarget ~= nil and GetUnitToUnitDistance(npcTarget, npcBot) <= AttackRange + 100) then
			return BOT_ACTION_DESIRE_MODERATE
		end
	end
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy) and GetUnitToUnitDistance(npcBot, npcEnemy) <= AttackRange + 100) then
				return BOT_ACTION_DESIRE_MODERATE
			end
		end
	end
...
Consider[4] = function()
	...
	local tableNearbyAttackingAlliedHeroes = utility.GetNearbyVisibleHeroes(npcBot, 1200, false, BOT_MODE_ATTACK)
...
Consider[5] = function()
	...
	if (ManaPercentage >= 0.4) then
		for k, creep in pairs(creepsNeutral) do
			if (IsGoodNeutralCreeps(creep) and not creep:WasRecentlyDamagedByAnyHero(1.5)) then

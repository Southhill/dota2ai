...
Consider[1] = function()
	...
	--protect myself
	if
		(ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana or
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		for i, npcEnemy in pairs(enemys) do
			if (CanCast[abilityNumber](npcEnemy)) then
				...
				if
					npcEnemy:GetHealth() <= npcEnemy:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL) * (0.85 + #allys * 0.05) and
						(npcEnemy:GetHealth() >= (ability:GetLevel() * 100 + 300) or
							AbilityExtensions:GetHealthPercent(npcEnemy) <= 0.45 and AbilityExtensions:GetHealthPercent(npcEnemy) >= 0.2)
				 then

...
Consider[1] = function()
	...
	local enemys = utility.GetNearbyVisibleHeroes(npcBot, 1599, true, BOT_MODE_NONE)
...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT) then
		if (#creeps >= 2 or #enemys >= 1) then
			if (npcBot:GetHealth() / npcBot:GetMaxHealth() < 0.4) then
				return BOT_ACTION_DESIRE_HIGH
			end
		end
	end

	if (npcBot:GetActiveMode() ~= BOT_MODE_NONE) then
		if (#creeps >= 2 or #enemys >= 1) then
			if (npcBot:GetHealth() / npcBot:GetMaxHealth() < 0.65 and ManaPercentage > 0.5) then
...
Consider[3] = function()
	...
	local allys = utility.GetNearbyVisibleHeroes(npcBot, 1599, false, BOT_MODE_NONE)
	...
	then
		if (WeakestAlly ~= nil) then
			if (AllyHealth / WeakestAlly:GetMaxHealth() < 0.3 + 0.4 * ManaPercentage) and not WeakestAlly:IsIllusion() then
	...
	for i, npcTarget in pairs(allys2) do
		if (npcTarget:IsAlive() and npcTarget:DistanceFromFountain() >= 1000) then
			local Threats = npcTarget:GetNearbyHeroes(900, true, BOT_MODE_NONE)
			local Towers = npcTarget:GetNearbyTowers(900, true)
			local TempFactor =
				(1 - npcTarget:GetHealth() / npcTarget:GetMaxHealth()) + (#Threats + #Towers) * 0.05 +
				0.05 * (5 - math.min(5, npcTarget:TimeSinceDamagedByAnyHero()))

			if TempFactor > HighestFactor then
				WeakestAlly2 = npcTarget
				HighestFactor = TempFactor
			end

			if (HighestFactor >= 0.4) then
				if not WeakestAlly2:IsIllusion() and CanCast[abilityNumber](npcTarget) then
	...
	if (ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana) then
		...
		for i = 0, 11, 1 do
			...
			if (tower ~= nil) then
				...
				if (creeps_t == nil) then
					creeps_t = {}
				end
				if (tower:IsAlive() and tower:GetHealth() / tower:GetMaxHealth() < LowestHP and #creeps_t <= 2) then
...
Consider[6] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_ATTACK) then
		if (#enemys + #allys >= 4 and #enemys >= 2) then
			return BOT_ACTION_DESIRE_HIGH
		end
	end
		...
		if (npcEnemy ~= nil) then
			if
				(npcEnemy:GetHealth() <= npcEnemy:GetActualIncomingDamage(npcBot:GetOffensivePower(), DAMAGE_TYPE_MAGICAL) and
					GetUnitToUnitDistance(npcEnemy, npcBot) <= Radius - 200)
			then
				return BOT_ACTION_DESIRE_MODERATE
			end
		end

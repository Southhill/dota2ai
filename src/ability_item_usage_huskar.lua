...
Consider[1] = function()
	...
	if
	 ...
	 then
		local tableNearbyEnemyHeroes = utility.GetNearbyVisibleHeroes(npcBot, Radius - 100, true, BOT_MODE_NONE)
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (GetUnitToUnitDistance(npcBot, npcEnemy) <= Radius - 150) then
...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (npcBot:GetHealth() / npcBot:GetMaxHealth() > 0.4 + 0.1 * #enemys and CreepHealth >= 150) then
			if (WeakestEnemy ~= nil) then
				if (CanCast[abilityNumber](WeakestEnemy)) then
					return BOT_ACTION_DESIRE_HIGH, WeakestEnemy
				end
	...
	if (npcBot:GetActiveMode() == BOT_MODE_FARM) then
		if (npcBot:GetHealth() / npcBot:GetMaxHealth() > 0.2 + 0.05 * #creeps) then
...
Consider[4] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if (HeroHealth <= WeakestEnemy:GetActualIncomingDamage(GetComboDamage(), DAMAGE_TYPE_MAGICAL) and #allys >= #enemys) then
					return BOT_ACTION_DESIRE_HIGH, WeakestEnemy
				end
			end
		end
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy) and GetUnitToUnitDistance(npcBot, npcEnemy) < CastRange and #allys >= #enemys) then
				return BOT_ACTION_DESIRE_MODERATE, npcEnemy
			end
		end
	end

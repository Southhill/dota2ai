...
local function CanCast2(npcTarget)
	if (utility.NCanCast(npcTarget) == false) then
		return false
	end

	if (npcTarget:GetTeam() == npcBot:GetTeam()) then
		...
		for i, ally in pairs(allys) do
			...
			if (AttackTarget ~= nil and AttackTarget == npcTarget) then
				IsAttacked = true
			end
		end
		return IsAttacked
...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (ManaPercentage > 0.65 or npcBot:GetMana() > ComboMana) then
			local locationAoE = npcBot:FindAoELocation(true, false, npcBot:GetLocation(), CastRange, Radius, 0, Damage)

			if (locationAoE.count >= 1 and GetUnitToLocationDistance(npcBot, locationAoE.targetloc) <= CastRange) then
				return BOT_ACTION_DESIRE_LOW, locationAoE.targetloc
			end
		end

		if ((ManaPercentage > 0.5 or npcBot:GetMana() > ComboMana) and ability:GetLevel() >= 3) then
			local locationAoE = npcBot:FindAoELocation(true, true, npcBot:GetLocation(), CastRange, Radius, 0, 0)
			if (locationAoE.count >= 4 and GetUnitToLocationDistance(npcBot, locationAoE.targetloc) <= CastRange) then
				return BOT_ACTION_DESIRE_LOW, locationAoE.targetloc
			end
		end
	end
...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if (WeakestEnemy:GetHealth() / WeakestEnemy:GetMaxHealth() < 0.5 and npcBot:GetMana() > ComboMana) then
...
Consider[3] = function()
	...
	if
	 ...
	 then
		if
			(tower ~= nil and GetUnitToUnitDistance(npcBot, tower[1]) >= 800) or
	...
	if
	 ...
	 then
		local npcTarget = npcBot:GetTarget()
		if (npcTarget ~= nil and GetUnitToUnitDistance(npcBot, npcTarget) <= 1400 and #enemys >= 2) then
			return GetAbilityPoint()
		end
	end
...
Consider[4] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if
					(WeakestEnemy:GetHealth() / WeakestEnemy:GetMaxHealth() < 0.8 or
						WeakestEnemy:HasModifier("modifier_pugna_decrepify")) and
						GetUnitToUnitDistance(npcBot, WeakestEnemy) < CastRange - 200
				 then
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy) and GetUnitToUnitDistance(npcBot, npcEnemy) < CastRange - 200) then

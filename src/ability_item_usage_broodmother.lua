...
Consider[4] = function()
	...
	local enemys = utility.GetNearbyVisibleHeroes(npcBot, math.min(1600, CastRange + 300), true, BOT_MODE_NONE)
...
function IsLocationOverlapWeb(location, Radius)
	...
	for _, u in pairs(unit) do
		if u:GetUnitName() == "npc_dota_broodmother_web" then
...
Consider[2] = function()
	...
	if
	 ...
	 then
		...
		if
			NearbyTower[1] ~= nil and not NearbyTower[1]:IsInvulnerable() and
...
Consider[1] = function()
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy) and GetUnitToUnitDistance(npcBot, npcEnemy) <= 400) then
				return BOT_ACTION_DESIRE_LOW
			end
		end
	end

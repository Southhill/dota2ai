...
local CanCast = {
	...
	function(t)
		return AbilityExtensions:SpellCanCast(t, true, true, true) and
			not AbilityExtensions:HasAbilityRetargetModifier(npcBot)
	end,
	utility.NCanCast,
	function(t)
...
Consider[1] = function()
	...
	--------------------------------------
	--protect myself
	if (npcBot:WasRecentlyDamagedByAnyHero(5) and npcBot:GetActiveMode() == BOT_MODE_RETREAT) then
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING or npcBot:GetActiveMode() == BOT_MODE_FARM) then
		if (WeakestCreep ~= nil) then
			if
			 ...
			 then
				if (CreepHealth <= WeakestCreep:GetActualIncomingDamage(Damage, DAMAGE_TYPE_PHYSICAL)) then
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if ((ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana) and ability:GetLevel() >= 2 and CreepHealth >= 150) then
			if (WeakestEnemy ~= nil) then
				if (CanCast[abilityNumber](WeakestEnemy)) then
...
Consider[2] = function()
	...
	if (npcBot:WasRecentlyDamagedByAnyHero(5) and npcBot:GetActiveMode() == BOT_MODE_RETREAT) then
		...
		for _, tempEnemy in pairs(allys) do
			...
			if (tempDistance < allydistance) then
				npcEnemy = tempEnemy
	...
	if
	 ...
	 then
		...
		if (ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana and #allys >= #enemys) then
			if (npcEnemy ~= nil) then
				if

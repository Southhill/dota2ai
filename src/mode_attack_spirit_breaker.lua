...
function Think()
	local npcBot = GetBot()
	if (npcBot:HasModifier("modifier_spirit_breaker_charge_of_darkness") == false) then
		npcBot.SBTarget = nil
	end
	if
		(npcBot:IsUsingAbility() or npcBot:IsChanneling() or npcBot:HasModifier("modifier_spirit_breaker_charge_of_darkness"))
	 then
		return
	end
	...
	if (npcEnemy ~= nil and npcEnemy:IsAlive()) then
		...
		if (npcBot:GetTarget() == nil and WeakestEnemy ~= nil) then

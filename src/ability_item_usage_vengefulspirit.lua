...
local CanCast = {
	function(target)
		return (npcBot:GetLevel() >= 25 and utility.UCanCast(target)) or
			utility.NCanCast(target) and not AbilityExtensions:HasAbilityRetargetModifier(target)
	end,
	utility.NCanCast,
	utility.NCanCast,
	function(target)
		return AbilityExtensions:NormalCanCast(target, false, DAMAGE_TYPE_MAGICAL, true) and
			not target:HasModifier("modifier_item_mirror_shield_delay") and
			not target:HasModifier("modifier_item_sphere_target")
	end
}

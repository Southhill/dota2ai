	...
	if
	 ...
	 then
		...
		for _, npcAlly in pairs(allys2) do
			...
			if (npcAlly:GetActiveMode() == BOT_MODE_RETREAT) then
				if (#enemys3 == 1) then
			...
			if (npcAlly:GetAssignedLane() == LANE_MID) then
				if (#enemys3 == 1) then
					local npcEnemy = enemys3[1]
		...
		for _, npcEnemy in pairs(enemys2) do
			...
			if (#enemys3 <= 2) then
				...
				if (npcEnemy:GetHealth() * 1.5 <= sumdamage and CanCast[abilityNumber](npcEnemy)) then
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil and HealthPercentage >= 0.7) then
			...
			local enemys3 = npcEnemy:GetNearbyHeroes(1600, false, BOT_MODE_NONE)
			if (CanCast[abilityNumber](npcEnemy) and not enemyDisabled(npcEnemy) and #enemys3 <= #allys3) then

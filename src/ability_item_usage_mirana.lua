...
Consider[2] = function()
	...
	local tableNearbyEnemyHeroes = utility.GetNearbyVisibleHeroes(npcBot, 1600, true, BOT_MODE_NONE)
	for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
		if (npcEnemy:IsChanneling() and not IsEnemyCreepBetweenMeAndTarget(npcBot, npcEnemy, npcEnemy:GetLocation(), nRadius)) then
...
Consider[3] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		...
		if (npcBot:WasRecentlyDamagedByAnyHero(2.0) and npcBot:IsFacingLocation(ancient:GetLocation(), 10)) then

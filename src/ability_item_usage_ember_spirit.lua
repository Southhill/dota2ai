...
Consider[1] = function()
    ...
    allNearbyTargettableEnemies =
        AbilityExtensions:Count(
        enemies,
        function(t)
            return not t:IsInvulnerable() and not t:IsMagicImmune()
        end
    )
    local fistBonus = 1
    if
        IsUsingSleightOfFist() and fistCastLocation ~= nil and
            GetUnitToLocationDistance(npcBot, fistCastLocation) >= Abilities[2]:GetCastRange() - 200
     then
        fistBonus = 2
    end
    local goodTargets =
    ...
    goodTargets =
        ...
        AbilityExtensions:Count(
            targettableEnemies,
            function(t)
                return AbilityExtensions:GetHealthPercent(t) <= duration / 5 + 0.1 * friendCount
            end
        )
...
Consider[2] = function()
    ...
    if
        not ability:IsFullyCastable() or IsUsingRemnant() or IsUsingSleightOfFist() or
            AbilityExtensions:CannotMove(npcBot) or
            ability:GetCurrentCharges() == 0
     then
        return 0
    end
    local abilityLevel = ability:GetLevel()
        ...
        if
            hasBattleFury and
                (locationAoE.count >= 4 and manaPercent >= 0.3 + manaCost or
                    locationAoE.count >= 3 and manaPercent >= 0.6 + manaCost)
         then
            return BOT_ACTION_DESIRE_LOW, locationAoE.targetloc
        end
    elseif AbilityExtensions:IsLaning(npcBot) then
        if
            manaPercent >= 0.6 + manaCost and #enemies == 1 and enemies[1]:CanBeSeen() and healthPercent >= 0.8 and
                abilityLevel >= 3
         then
            return BOT_ACTION_DESIRE_HIGH, AbilityExtensions:FindAOELocationAtSingleTarget(
                npcBot,
                enemies[1],
                radius,
                castRange,
        ...
        if
            manaPercent >= 0.3 and #enemies == 1 and enemies[1]:CanBeSeen() and healthPercent >= 0.75 and
                abilityLevel >= 4
         then
            return BOT_ACTION_DESIRE_HIGH, AbilityExtensions:FindAOELocationAtSingleTarget(
                npcBot,
    ...
    if AbilityExtensions:NotRetreating(npcBot) then
        ...
        if weakestEnemy ~= nil and enemyHealth ~= -1 then
            ...
            if enemyHealth <= damageTaken or enemyHealth <= damageTaken * 1.5 and mana > 200 + manaCost then
                local targetLocation =
                    AbilityExtensions:FindAOELocationAtSingleTarget(npcBot, weakestEnemy, radius, castRange, castPoint)
        ...
        if
            target ~= nil and target:CanBeSeen() and AbilityExtensions:NormalCanCast(target, true, DAMAGE_TYPE_PHYSICAL) and
                not AbilityExtensions:CannotBeAttacked(target) and
                (mana >= 200 and
                    target:GetHealth() <
                        target:GetActualIncomingDamage(damage, DAMAGE_TYPE_PHYSICAL) * (0.9 + friendCount * 0.1))
         then
            return BOT_ACTION_DESIRE_HIGH, AbilityExtensions:FindAOELocationAtSingleTarget(
                npcBot,
                target,
...
Consider[3] = function()
    ...
    if
        not ability:IsFullyCastable() or npcBot:HasModifier("modifier_ember_spirit_flame_guard") or
    ...
    elseif AbilityExtensions:IsLaning(npcBot) then
        if #enemyCreeps >= 3 and mana >= npcBot:GetMaxMana() * 0.3 + manaCost then
            ...
            if
                laneEnemyHero ~= nil and GetUnitToUnitDistance(npcBot, laneEnemyHero) <= radius and
                    AbilityExtensions:GetHealthPercent(laneEnemyHero) <= 0.4
             then
                return BOT_ACTION_DESIRE_MODERATE
            end
    ...
    elseif AbilityExtensions:IsAttackingEnemies(npcBot) then
        ...
        if IsUsingSleightOfFist() then
            ...
            if
                #enemies >= 1 and DotaTime() < 10 * 60 or
                    #realEnemies >= 2 and npcBot:GetNetWorth() / DotaTime() * 60 <= 350 or
                    #realEnemies >= 3
             then
                return BOT_ACTION_DESIRE_HIGH
            end
    ...
    do
        local target = npcBot:GetTarget()
        if
            target ~= nil and target:CanBeSeen() and not AbilityExtensions:NormalCanCast(target) and
                GetUnitToUnitDistance(npcBot, target) < radius - 50 and
                abilityLevel >= 3
         then
            return BOT_ACTION_DESIRE_MODERATE
        end
...
Consider[4] = function()
    ...
    if
        not ability:IsFullyCastable() or IsUsingSleightOfFist() or IsUsingRemnant() or
            AbilityExtensions:CannotMove(npcBot)
     then
        return 0
    end
    DetectRemnant()
    ...
    if
        healthPercent >= 0.75 and manaPercent >= 0.9 and AbilityExtensions:GetDistanceFromAncient(npcBot) <= 1000 and
            DotaTime() <= 15 * 60
     then
        if AbilityExtensions:IsLaning(npcBot) then
            local laneFront = GetLaneFrontLocation(GetTeam(), npcBot:GetAssignedLane(), 0)
            local remnantUnderTower =
                AbilityExtensions:Filter(
                activeRemnants,
                function(t)
                    return GetUnitToUnitDistance(t, laneFront) <= 1000
                end
            )
        ...
        elseif AbilityExtensions:IsFarmingOrPushing(npcBot) then
            local remnantUnderTower =
                ...
                activeRemnants,
                function(t)
                    return GetUnitToUnitDistance(t, npcBot) >= 4000 and
                        #AbilityExtensions:GetNearbyNonIllusionHeroes(t) == 0
                end
            )
            if remnantUnderTower[1] then
                return BOT_ACTION_DESIRE_MODERATE, remnantUnderTower[1]:GetLocation()
        ...
        elseif AbilityExtensions:IsAttackingEnemies(npcBot) then
            for _, activeRemnant in ipairs(activeRemnants) do
                ...
                local friendCount = AbilityExtensions:GetEnemyHeroNumber(npcBot, friends)
                local veryNearEnemies =
                    AbilityExtensions:GetEnemyHeroNumber(npcBot, AbilityExtensions:GetNearbyHeroes(activeRemnant, 400))
    ...
    if AbilityExtensions:IsRetreating(npcBot) then
        local distanceToFountain =
            AbilityExtensions:Filter(
            activeRemnants,
            function(t)
                local distance = GetUnitToUnitDistance(t, npcBot)
                return distance >= npcBot:GetCurrentMovementSpeed() * 2 and
                    distance > AbilityExtensions:GetDistanceFromAncient(t) + 600
            end
        )
        distanceToFountain =
        ...
        for _, activeRemnant in ipairs(activeRemnants) do
            ...
            if
                target ~= nil and target:CanBeSeen() and AbilityExtensions:NormalCanCast(target, false) and
                    GetUnitToUnitDistance(activeRemnant, target) <= radius
             then
            ...
            local realEnemies =
                ...
                enemies,
                function(t)
                    return AbilityExtensions:MayNotBeIllusion(npcBot, t) and AbilityExtensions:NormalCanCast(t, false)
                end
            )
        end
    end
...
local function ShouldGoToFountainFromLane()
    return healthPercent <= 0.25 and not AbilityExtensions:HasAvailableItem("item_flask") or
...
Consider[5] = function()
    ...
    if AbilityExtensions:IsFarmingOrPushing(npcBot) and AbilityExtensions:GetManaPercent(npcBot) >= 0.9 then
        ...
        if
            #nearbyTowers == 1 and
                (nearbyTowers[1] == GetTower(GetTeam(), TOWER_MID_1) or
                    nearbyTowers[1] == GetTower(GetTeam(), TOWER_MID_2))
         then
    ...
    if AbilityExtensions:IsRetreating(npcBot) then
        ...
        if npcBot:WasRecentlyDamagedByAnyHero(1.5) and #enemies ~= 0 then
            ...
            local extraLocation =
                npcBot:GetLocation() + Vector(useRange * math.cos(strangeFacing), useRange * math.sin(strangeFacing))
            if npcBot:HasScepter() then
                extraLocation =
                    AbilityExtensions:GetPointFromLineByDistance(
                    npcBot:GetLocation(),
                    AbilityExtensions:GetAncient(npcBot),
                    useRange
                )
            end
            if
                npcBot:GetCurrentMovementSpeed() >= 285 or
                    npcBot:GetCurrentMovementSpeed() >= 240 and IsUsingSleightOfFist()
             then
        ...
        if target ~= nil and target:CanBeSeen() and AbilityExtensions:NormalCanCast(target, false) then
            if AbilityExtensions:GetHealthPercent(target) <= 0.7 then
                ...
                if target:GetHealth() <= (0.9 + 0.18 * friendCount) * damageOnce * remnantNumber + charge then
                    return BOT_ACTION_DESIRE_HIGH, target:GetExtrapolatedLocation(
                        GetUnitToUnitDistance(target, npcBot) /

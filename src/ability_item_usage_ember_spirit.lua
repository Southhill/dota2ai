...
Consider[1] = function()
    ...
    local realEnemies = AbilityExtensions:Filter(enemies, function(t)
        return AbilityExtensions:MayNotBeIllusion(npcBot, t)
    end)
    local targettableEnemies = AbilityExtensions:Filter(realEnemies, function(t)
    ...
    local weakCreeps = AbilityExtensions:Filter(enemyCreeps, function(t)
    ...
    allNearbyTargettableEnemies = AbilityExtensions:Count(enemies, function(t)
        return not t:IsInvulnerable() and not t:IsMagicImmune()
    end)
    local fistBonus = 1
    if IsUsingSleightOfFist() and fistCastLocation ~= nil and GetUnitToLocationDistance(npcBot, fistCastLocation) >=
        Abilities[2]:GetCastRange() - 200 then
        fistBonus = 2
    end
    local goodTargets = AbilityExtensions:Count(targettableEnemies, function(t)
    ...
    goodTargets = goodTargets + AbilityExtensions:Count(targettableEnemies, function(t)
...
Consider[2] = function()
    ...
    if not ability:IsFullyCastable() or IsUsingRemnant() or IsUsingSleightOfFist() or
        AbilityExtensions:CannotMove(npcBot) then
        return 0
    end
    local abilityLevel = ability:GetLevel()
    ...
    projectiles = AbilityExtensions:Any(projectiles, function(t)
    ...
    if AbilityExtensions:IsFarmingOrPushing(npcBot) then
        ...
        if hasBattleFury and
            (locationAoE.count >= 4 and manaPercent >= 0.3 + manaCost or locationAoE.count >= 3 and manaPercent >= 0.6 +
                manaCost) then
    ...
    elseif AbilityExtensions:IsLaning(npcBot) then
        if manaPercent >= 0.6 + manaCost and #enemies == 1 and enemies[1]:CanBeSeen() and healthPercent >= 0.8 and
            abilityLevel >= 3 then
            return BOT_ACTION_DESIRE_HIGH,
                AbilityExtensions:FindAOELocationAtSingleTarget(npcBot, enemies[1], radius, castRange, castPoint)
        end
        if manaPercent >= 0.3 and #enemies == 1 and enemies[1]:CanBeSeen() and healthPercent >= 0.75 and abilityLevel >=
            4 then
            return BOT_ACTION_DESIRE_HIGH,
                AbilityExtensions:FindAOELocationAtSingleTarget(npcBot, enemies[1], radius, castRange, castPoint)
        end
    ...
    if AbilityExtensions:NotRetreating(npcBot) then
        ...
        if weakestEnemy ~= nil and enemyHealth ~= -1 then
            ...
            if enemyHealth <= damageTaken or enemyHealth <= damageTaken * 1.5 and mana > 200 + manaCost then
                local targetLocation = AbilityExtensions:FindAOELocationAtSingleTarget(npcBot, weakestEnemy, radius,
                    castRange, castPoint)
        ...
        if target ~= nil and target:CanBeSeen() and AbilityExtensions:NormalCanCast(target, true, DAMAGE_TYPE_PHYSICAL) and
            not AbilityExtensions:CannotBeAttacked(target) and
            (mana >= 200 and target:GetHealth() < target:GetActualIncomingDamage(damage, DAMAGE_TYPE_PHYSICAL) *
                (0.9 + friendCount * 0.1)) then
            return BOT_ACTION_DESIRE_HIGH,
...
Consider[3] = function()
    ...
    elseif AbilityExtensions:IsLaning(npcBot) then
        if #enemyCreeps >= 3 and mana >= npcBot:GetMaxMana() * 0.3 + manaCost then
            ...
            if laneEnemyHero ~= nil and GetUnitToUnitDistance(npcBot, laneEnemyHero) <= radius and
                AbilityExtensions:GetHealthPercent(laneEnemyHero) <= 0.4 then
                return BOT_ACTION_DESIRE_MODERATE
            end
    ...
    elseif AbilityExtensions:IsAttackingEnemies(npcBot) then
        ...
        if IsUsingSleightOfFist() then
            local sleightFistMarkedHeroes = AbilityExtensions:Count(enemies, function(t)
            ...
            if #enemies >= 1 and DotaTime() < 10 * 60 or #realEnemies >= 2 and npcBot:GetNetWorth() / DotaTime() * 60 <=
                350 or #realEnemies >= 3 then
                return BOT_ACTION_DESIRE_HIGH
            end
        end
    elseif AbilityExtensions:IsRetreating(npcBot) then
    ...
    do
        local target = npcBot:GetTarget()
        if target ~= nil and target:CanBeSeen() and not AbilityExtensions:NormalCanCast(target) and
            GetUnitToUnitDistance(npcBot, target) < radius - 50 and abilityLevel >= 3 then
            return BOT_ACTION_DESIRE_MODERATE
        end
    end
end
...
local function RefreshActiveRemnants()
    activeRemnants = AbilityExtensions:Filter(GetUnitList(UNIT_LIST_ALL), function(t)
...
Consider[4] = function()
    ...
    if not ability:IsFullyCastable() or IsUsingSleightOfFist() or IsUsingRemnant() or
        AbilityExtensions:CannotMove(npcBot) then
        return 0
    end
    DetectRemnant()
    ...
    if healthPercent >= 0.75 and manaPercent >= 0.9 and AbilityExtensions:GetDistanceFromAncient(npcBot) <= 1000 and
        DotaTime() <= 15 * 60 then
        if AbilityExtensions:IsLaning(npcBot) then
        ...
        elseif AbilityExtensions:IsAttackingEnemies(npcBot) then
            for _, activeRemnant in ipairs(activeRemnants) do
                ...
                local friendCount = AbilityExtensions:GetEnemyHeroNumber(npcBot, friends)
                local veryNearEnemies = AbilityExtensions:GetEnemyHeroNumber(npcBot, AbilityExtensions:GetNearbyHeroes(
                    activeRemnant, 400))
    ...
    if AbilityExtensions:IsRetreating(npcBot) then
        ...
        for _, activeRemnant in ipairs(activeRemnants) do
            ...
            if target ~= nil and target:CanBeSeen() and AbilityExtensions:NormalCanCast(target, false) and
...
Consider[5] = function()
    ...
    if AbilityExtensions:IsRetreating(npcBot) then
        ...
        if npcBot:WasRecentlyDamagedByAnyHero(1.5) and #enemies ~= 0 then
            ...
            if npcBot:GetCurrentMovementSpeed() >= 285 or npcBot:GetCurrentMovementSpeed() >= 240 and
                IsUsingSleightOfFist() then

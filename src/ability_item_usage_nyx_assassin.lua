...
Consider[1] = function()
	...
	do
		...
		for _, npcEnemy in pairs(enemys2) do
			if (CanCast[abilityNumber](npcEnemy) and npcBot:WasRecentlyDamagedByHero(npcEnemy, 2.0)) then
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy) and npcBot:GetMana() > ComboMana) then
...
Consider[3] = function()
	...
	--Try to kill enemy hero
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil and CanCast[abilityNumber](WeakestEnemy)) then
			if (WeakestEnemy:GetHealth() / WeakestEnemy:GetMaxHealth() < 0.3) then
	...
	--------------------------------------
	--protect myself
	local enemys2 = utility.GetNearbyVisibleHeroes(npcBot, 600, true, BOT_MODE_NONE)
	-- If we're seriously retreating, see if we can land a stun on someone who's damaged us recently
	if
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (npcEnemy:GetHealth() / npcEnemy:GetMaxHealth() < 0.4) then

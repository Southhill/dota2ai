...
Consider[2] = function()
	...
	--------------------------------------
	--protect myself
	if (#enemys - #creeps >= 1 or (npcBot:GetActiveMode() == BOT_MODE_RETREAT and #enemys >= 1)) then
		if (ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana) then
			for _, npcEnemy in pairs(enemys) do
				if (CanCast[abilityNumber](npcEnemy)) then
					return BOT_ACTION_DESIRE_HIGH
...
Consider[5] = function()
	...
	if (ability:GetToggleState() == true and #creeps + #enemys == 0) then
		return BOT_ACTION_DESIRE_HIGH
	end
	...
	if (#tableNearbyAttackingAlliedHeroes >= 2 and #enemys >= 2) then
		return BOT_ACTION_DESIRE_MODERATE
	end

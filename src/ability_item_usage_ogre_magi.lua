...
Consider[1] = function()
	...
	for _, enemy in pairs(enemys) do
		if (enemy:IsChanneling() and CanCast[1](enemy)) then
	...
	if (#tableNearbyAttackingAlliedHeroes >= 2) then
		...
		for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
			if (CanCast[1](npcEnemy) and not enemyDisabled(npcEnemy)) then
	...
	end -- If we're going after someone
	--
	--------------------------------------
	-- Mode based usage
	--------------------------------------
...
Consider[2] = function()
	...
	if
	 ...
	 then
		...
		if (#creeps >= 3) then
			if (ManaPercentage > 0.5 or npcBot:GetMana() > ComboMana and AbilitiesReal[4]:GetLevel() >= 1) then
				if (CanCast[2](creeps[1]) and GetUnitToUnitDistance(npcBot, creeps[1]) < CastRange + 75 * #allys) then
...
Consider[3] = function()
	...
	if
	 ...
	 then
		if (ManaPercentage > 0.6 and npcBot:GetMana() > ComboMana) then
			...
			for _, myTower in pairs(tableNearbyFriendlyTowers) do
				if (GetUnitToUnitDistance(myTower, npcBot) < CastRange and not myTower:HasModifier("modifier_ogre_magi_bloodlust")) then
...
Consider[4] = function()
	...
	if
		not ability:IsFullyCastable() or AbilitiesReal[1]:IsFullyCastable() or
			ability:GetManaCost() / npcBot:GetMaxMana() > 0.3
	 then
		return BOT_ACTION_DESIRE_NONE, 0
	end
	...
	if (#tableNearbyAttackingAlliedHeroes >= 2) then
		...
		local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes(CastRange, true, BOT_MODE_NONE)
		for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
			if (CanCast[5](npcEnemy) and not enemyDisabled(npcEnemy)) then
				local Damage2 = npcEnemy:GetEstimatedDamageToTarget(false, npcBot, 3.0, DAMAGE_TYPE_ALL)
				if (Damage2 > nMostDangerousDamage) then
					nMostDangerousDamage = Damage2
					npcMostDangerousEnemy = npcEnemy
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[5](WeakestEnemy)) then
				if
					(HeroHealth <= WeakestEnemy:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL) or
						(HeroHealth <= WeakestEnemy:GetActualIncomingDamage(GetComboDamage(), DAMAGE_TYPE_MAGICAL) and
							npcBot:GetMana() > ComboMana))

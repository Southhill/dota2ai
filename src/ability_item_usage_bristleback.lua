...
local Consider = {}
local CanCast = {
	bristleback_viscous_nasal_goo_CanCast,
	utility.NCanCast,
	utility.NCanCast,
	utility.UCanCast
}
...
Consider[1] = function()
	...
	if AbilityExtensions:HasScepter(npcBot) then
		...
		if
		 ...
		 then
			...
			return #enemies >= 2 and AbilityExtensions:GetManaPercent(npcBot) >= 0.3 or
				#enemies == 1 and AbilityExtensions:GetManaPercent(npcBot) >= 0.6 or
				npcBot:WasRecentlyDamagedByAnyHero(1.5)
		end
		return 0
	end
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy) and GetUnitToUnitDistance(npcBot, WeakestEnemy) < CastRange) then
...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (WeakestCreep ~= nil) then
			if ((ManaPercentage > 0.5 or npcBot:GetMana() > ComboMana) and GetUnitToUnitDistance(npcBot, WeakestCreep) < Radius) then

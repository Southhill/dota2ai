...
Consider[1] = function()
	...
	if
	 ...
	 then
		...
		if (locationAoE.count >= 3 and #enemys >= 2) then
			return BOT_ACTION_DESIRE_LOW, locationAoE.targetloc
		end
	end
...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil and npcBot:HasModifier("modifier_weaver_shukuchi")) then
...
Consider[4] = function()
	...
	if (npcBot.WeaverSkill4 == nil or DotaTime() - npcBot.WeaverSkill4.Timer >= 2) then
		npcBot.WeaverSkill4 = {Hp = HealthPercentage, Timer = DotaTime()}
	end

	-- If we suffered a lot of damage
	if (npcBot.WeaverSkill4.Hp - HealthPercentage >= 0.4 + 0.05 * #enemys) then
		return BOT_ACTION_DESIRE_HIGH, utility.GetUnitsTowardsLocation(npcBot, GetAncient(GetTeam()), CastRange)
	end

	-- If we're in a teamfight, use it on the scariest enemy
	local tableNearbyAttackingAlliedHeroes = utility.GetNearbyVisibleHeroes(npcBot, 800, false, BOT_MODE_ATTACK)
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		...
		for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
			if (npcBot:WasRecentlyDamagedByHero(npcEnemy, 2.0)) then
				if (CanCast[abilityNumber] and HealthPercentage < 0.25) then
					return BOT_ACTION_DESIRE_HIGH
				end
			end
		end

...
function GetDesire() -- --
	...
	--[[if bot.lastPlayerChat ~= nil and string.find(bot.lastPlayerChat.text, "ward") then
		...
		bot:IsChanneling() or bot:IsIllusion() or bot:IsInvulnerable() or not bot:IsHero() or not IsSuitableToWard() or
			bot:GetCurrentActionType() == BOT_ACTION_TYPE_IDLE
	 then
	...
	if itemWard ~= nil then
		...
		--[[if bot.lastPlayerChat ~= nil and string.find(bot.lastPlayerChat.text, "ward") then
			...
			wardUtils.GetAvailableSpot(bot)
...
function IsSuitableToWard()
	...
	if
		((mode == BOT_MODE_RETREAT and bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) or mode == BOT_MODE_ATTACK or
			mode == BOT_MODE_RUNE or
...
function IsEnemyCloserToWardLoc(wardLoc, botDist)
	...
	for i = 1, #enemyPids do
		...
		if info ~= nil then
			local dInfo = info[1]
			if dInfo ~= nil and dInfo.time_since_seen < 3.0 and utility.GetDistance(dInfo.location, wardLoc) < botDist then
				return true
			end
		end
	end

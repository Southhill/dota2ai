...
function GetTeamRoamDesire()
	...
	if (CheckTimer == nil or CheckTimer > DotaTime()) then
		CheckTimer = DotaTime()
	end

	if (DotaTime() - CheckTimer > 5 and npcBot.TeamRoam ~= true and npcBot.TeamRoamTimer == nil) then
		ConsiderTeamRoam()
		CheckTimer = DotaTime()
	end

	if (npcBot.TeamRoam == true) then
		return 0.7
	end
...
function ConsiderTeamRoam()
	...
	if (item_smoke ~= nil and GetAllyFactor(npcBot) >= 0.7) then
...
function GetEnemyFactor(npcEnemy, allys)
	local npcBot = GetBot()
	if (GetUnitToUnitDistance(npcBot, npcEnemy) >= 3000) then
		local TowersCount = 0
		...
		for _, enemy in pairs(enemys2) do
			if (enemy:GetFactor() >= 1.0) then
				EnemysCount = EnemysCount + 1
			end
		end

		if (TowersCount > 0 or EnemysCount - AllysCount >= 1) then
			return 0, 0
		end
		...
		for i, npcAlly in pairs(allys3) do
			...
			for i, npcAlly2 in pairs(allys) do
				if (npcAlly:GetPlayerID() == npcAlly2:GetPlayerID()) then
					IsIn = true
				end
			end
			if (IsIn == false) then
		...
		for i, npcAlly in pairs(allys) do
			if (GetUnitToUnitDistance(npcAlly, npcEnemy) >= 1600) then
...
function TeamRoamThink()
	...
	if (npcBot.TeamRoam == true) then
		if (IsHeroAlive(npcBot.TeamRoamTargetID) == false or DotaTime() - npcBot.TeamRoamTimer >= 40 or #towers >= 1) then
	...
	if (npcBot.TeamRoamState == "Assemble") then
		if (npcBot.TeamRoamLeader:GetUnitName() == npcBot:GetUnitName()) then
			...
			if (#enemys > 0) then
				ready = false
				npcBot.TeamRoamAssemblyPoint =
					GetLocationTowardsLocation(npcBot.TeamRoamAssemblyPoint, GetAncient(GetTeam()):GetLocation(), 100)
			end

			for _, npcAlly in pairs(GetUnitList(UNIT_LIST_ALLIED_HEROES)) do
				if (IsPlayerBot(npcAlly:GetPlayerID()) == true and npcAlly.TeamRoam == true) then
					if (GetUnitToUnitDistance(npcBot, npcAlly) > 1000) then
						ready = false
					end
				end
			end
			...
			if (ready == true) then
				local item_smoke = utility.IsItemAvailable("item_smoke_of_deceit")
				if (npcBot:HasModifier("modifier_smoke_of_deceit") == false) then
			...
			if (GetUnitToUnitDistance(npcBot, npcBot.TeamRoamLeader) > 300) then
				npcBot:Action_MoveToLocation(npcBot.TeamRoamLeader:GetLocation())
			end
		end

		if (npcBot:HasModifier("modifier_smoke_of_deceit")) then
			npcBot.TeamRoamState = "Roaming"
			npcBot.TeamRoamTimer = DotaTime()
		end
	elseif (npcBot.TeamRoamState == "Roaming") then
		local enemys3 = GetUnitList(UNIT_LIST_ENEMY_HEROES)
		local target
		for _, enemy in pairs(enemys3) do
			if (enemy:GetPlayerID() == npcBot.TeamRoamTargetID) then
				target = enemy
			end
		end
		...
		if (seeninfo ~= nil) then
			...
			if (seentime > 5 and npcBot.TeamRoamLeader:GetUnitName() == npcBot:GetUnitName()) then
			...
			if (GetUnitToLocationDistance(npcBot, seenpoint) <= 1200) then
				if (target ~= nil) then
				...
				for _, npcAlly in pairs(GetUnitList(UNIT_LIST_ALLIED_HEROES)) do
					if (IsPlayerBot(npcAlly:GetPlayerID()) == true and npcAlly.TeamRoam == true) then
						if (GetUnitToUnitDistance(npcBot, npcBot.TeamRoamLeader) > 500) then
							ready = false
						end
					end
				end

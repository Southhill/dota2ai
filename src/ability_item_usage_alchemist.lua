...
function ConsiderSoulRing()
	...
	if (soulring ~= nil and soulring:IsFullyCastable() and HealthPercentage > 0.6) then
...
Consider[1] = function()
	...
	if (ability:IsCooldownReady() and (#creeps > 0 or #enemys > 0)) then
		ConsiderSoulRing()
	end
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy) and enemyDisabled(npcEnemy)) then
...
Consider[5] = function()
	...
	-- If we're seriously retreating, see if we can land a stun on someone who's damaged us recently
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		if (npcBot:WasRecentlyDamagedByAnyHero(2.0) and HealthPercentage <= 0.70 + #enemys * 0.05) then
			return BOT_ACTION_DESIRE_HIGH
		end
	end
...
Consider[4] = function()
	...
	local allys = utility.GetNearbyVisibleHeroes(npcBot, CastRange + 200, false, BOT_MODE_NONE)
	...
	local checkAlly = function(ally)
		...
		--------------------------------------
		-- If we're seriously retreating, see if we can land a stun on someone who's damaged us recently
		if (ally:GetActiveMode() == BOT_MODE_RETREAT and ally:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
			if (ally:WasRecentlyDamagedByAnyHero(2.0) and HealthPercentage <= 0.70 + #enemys * 0.05) then
				return BOT_ACTION_DESIRE_HIGH
			end
		end
...
Consider[6] = function()
	...
	if (npcBot:GetHealth() / npcBot:GetMaxHealth() <= 0.25 and TimeSinceCast >= 1) then
		local es = utility.GetNearbyVisibleHeroes(npcBot, CastRange, true, BOT_MODE_NONE)
		if #es > 0 then
...
local function GetNonScepterFriends()
	...
	return AbilityExtensions:FilterNot(
		friends,
		function(t)
			return HasRealScepter(t)
		end
	)
end
...
local function CheckFeedScepter()
	...
	AbilityExtensions:ForEach(
		friends,
		function(t)
			coroutine.yield(GetFeedScepterDesire(t), t)
		end
	)
end

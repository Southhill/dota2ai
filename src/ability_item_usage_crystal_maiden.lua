...
local CanCast = {
	utility.NCanCast,
	function(t)
		return AbilityExtensions:NormalCanCast(t, false, DAMAGE_TYPE_MAGICAL, false, true) and
	...
	if (npcBot:GetActiveMode() == BOT_MODE_FARM) then
		if
			(CreepHealth2 >= 600 and not StrongestCreep:GetUnitName() == "npc_dota_neutral_mud_golem" and
				not StrongestCreep:IsAncientCreep() and
				npcBot:GetMana() > 240) and
	...
	if (npcBot:IsUsingAbility() or npcBot:IsChanneling() or npcBot:IsSilenced()) then
		...
		if npcBot:IsChanneling() and npcBot:GetCurrentActiveAbility() == AbilitiesReal[4] then
			if not AbilityExtensions:IsFarmingOrPushing(npcBot) then
				...
				local enemies = utility.GetNearbyVisibleHeroes(npcBot, AbilitiesReal[4]:GetAOERadius(), true, BOT_MODE_NONE)
				if #enemies > 0 or freezingFieldHitSomeoneTimer == nil then
					freezingFieldHitSomeoneTimer = DotaTime()
				else
					if
						DotaTime() - freezingFieldHitSomeoneTimer >= 1.5 and
							#utility.GetNearbyVisibleHeroes(npcBot, AbilitiesReal[4]:GetAOERadius() + 200, false, BOT_MODE_NONE) > 0
					 then

...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (ManaPercentage > 0.8 or npcBot:GetMana() > ComboMana) then
			if (WeakestEnemy ~= nil) then
				if (CanCast[abilityNumber](WeakestEnemy)) then
					local target = UseAt(WeakestEnemy)
					if target ~= nil then
...
Consider[2] = function()
	...
	if enemyAxe then
		canEnemyAxeUseCullingBlade =
			AbilityExtensions:GetSilenceRemainingDuration(enemyAxe) <= 3 and
...
Consider[3] = function()
	...
	local WeakestAlly, AllyHealth = utility.GetWeakestUnit(allys)
	local enemys = utility.GetNearbyVisibleHeroes(npcBot, CastRange + 200, true, BOT_MODE_NONE)
	local WeakestEnemy, HeroHealth = utility.GetWeakestUnit(enemys)
	local creeps = npcBot:GetNearbyCreeps(CastRange + 300, false)
	local WeakestCreep, CreepHealth = utility.GetWeakestUnit(creeps)

	for _, npcEnemy in pairs(enemys) do
		if (npcEnemy:CanBeSeen()) then
			...
			if (Target ~= nil) then
				...
				if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
					if (CanCast[abilityNumber](npcEnemy)) then
						if (npcEnemy:GetHealth() <= npcEnemy:GetActualIncomingDamage(RealDamage, DamageType)) then
							return BOT_ACTION_DESIRE_HIGH, Target
						end
					end
				end
				...
				if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
					if (ManaPercentage > 0.4 and RadiusCount >= 3) then
						if (CanCast[abilityNumber](npcEnemy)) then
	...
	for _, npcTarget in pairs(allys) do
		...
		if (npcTarget:GetHealth() / npcTarget:GetMaxHealth() < HpFactor) then
	...
	-- If we're farming
	if
		(npcBot:GetActiveMode() == BOT_MODE_FARM or npcBot:GetActiveMode() == BOT_MODE_PUSH_TOWER_TOP or

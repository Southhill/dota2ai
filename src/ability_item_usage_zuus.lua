...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[1](WeakestEnemy)) then
				if
	...
	if
	 ...
	 then
		if (#enemys + #creeps >= 3 and ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana) then
			...
			if (WeakestCreep ~= nil) then
				if (CanCast[1](WeakestCreep) and GetUnitToUnitDistance(npcBot, WeakestCreep) < CastRange + 75 * #allys) then
	 ...
	 then
		...
		if (ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana) then
			if (npcTarget ~= nil) then
				if (CanCast[1](npcTarget) and GetUnitToUnitDistance(npcBot, npcTarget) < CastRange + 75 * #allys) then
...
Consider[2] = function()
	...
	for _, enemy in pairs(enemys) do
		if (enemy:IsChanneling() and CanCast[2](enemy) and not enemy:HasModifier("modifier_antimage_counterspell")) then
	...
	if
	 ...
	 then
		...
		if (npcTarget ~= nil) then
			if (CanCast[2](npcTarget) and GetUnitToUnitDistance(npcBot, npcTarget) < CastRange + 75 * #allys) then
...
Consider[6] = function()
	...
	for _, Enemy in pairs(GetUnitList(UNIT_LIST_ENEMY_HEROES)) do
		if Enemy ~= nil then
			if (GetUnitToUnitDistance(npcBot, Enemy) > AbilitiesReal[2]:GetCastRange() or not AbilitiesReal[2]:IsFullyCastable()) then
				if Enemy:IsAlive() and Enemy:CanBeSeen() and CanCast[4](Enemy) then
					if (LowestHP > Enemy:GetHealth()) then
						WeakestEnemy = Enemy
...
Consider[4] = function()
	...
	if (tower ~= nil and GetUnitToUnitDistance(npcBot, tower[1]) < CastRange + 500) then

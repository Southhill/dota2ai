...
Consider[1] = function()
	...
	if XMarksEnemy() and CanCast[1](xMarkTarget) and DotaTime() - xMarkTime <= 0.8 then
		print(
			"Torrent on x marked target: " ..
				xMarkTarget:GetUnitName() ..
					" at location " .. AbilityExtensions:ToStringVector(xMarkLocation) .. " at " .. DotaTime()
		)
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy) and WeakestEnemy:HasModifier("modifier_kunkka_x_marks_the_spot")) then
				if
					WeakestEnemy:GetModifierRemainingDuration(WeakestEnemy:GetModifierByName("modifier_kunkka_x_marks_the_spot")) < 1.6
				 then
...
Consider[3] = function()
	...
	if (npcTarget ~= nil) then
		if (CanCast[3](npcTarget)) then
			if
				(GetComboDamage() * (0.85 + 0.15 * #allys) > npcTarget:GetHealth() and
		...
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if
				 ...
				 then
					if
						not AbilitiesReal[1]:IsFullyCastable() or
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy)) then
				if
					not AbilitiesReal[1]:IsFullyCastable() or
						npcEnemy:GetModifierRemainingDuration(npcEnemy:GetModifierByName("modifier_kunkka_x_marks_the_spot")) < 1.2
				 then
...
Consider[7] = function()
	...
	local ability = AbilitiesReal[abilityNumber]
	if
		not ability:IsFullyCastable() or ability:IsHidden() or xMarkTarget == nil or
			not xMarkTarget:HasModifier("modifier_kunkka_x_marks_the_spot")
	 then
		return 0
	end
	...
	if
		XMarksEnemy() and npcBot:GetActiveMode() == BOT_MODE_RETREAT and
			(GetUnitToUnitDistance(npcBot, xMarkTarget) <= 300 or npcBot:WasRecentlyDamagedByHero(xMarkTarget, 1)) and
			DotaTime() > 1 + xMarkTime and
			GetUnitToLocationDistance(xMarkTarget, xMarkLocation)
	 then
		return BOT_ACTION_DESIRE_HIGH
	end
	...
	if
		xMarkTarget ~= nil and
			(DotaTime() > xMarkTime + xMarkDuration or not npcBot:IsAlive() or not xMarkTarget:IsAlive() or
				not xMarkTarget:HasModifier("modifier_kunkka_x_marks_the_target"))
	 then
		xMarkTarget = nil
		xMarkTime = nil
		xMarkLocation = nil

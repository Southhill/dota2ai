	...
	if
	 ...
	 then
		for k, npcEnemy in pairs(enemys) do
			if (enemyDisabled(npcEnemy) or npcEnemy:GetCurrentMovementSpeed() < MaxSpeed) then
...
function Consider4()
	...
	for _, npcEnemy in pairs(enemys) do
		...
		if ((Health - (Damage + Duration * DotDamage) * (1 - MagicResist)) / MaxHealth < KillPectange * 0.01) then
			if (CanCast[abilityNumber](npcEnemy)) then

...
Consider[1] = function()
	...
	local enemys = utility.GetNearbyVisibleHeroes(npcBot, CastRange + Radius, true, BOT_MODE_NONE)
	...
	if (npcBot:GetActiveMode() == BOT_MODE_ATTACK or npcBot:GetActiveMode() == BOT_MODE_LANING) then
	...
	then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy) and GetUnitToUnitDistance(npcBot, npcEnemy) <= CastRange + Radius) then

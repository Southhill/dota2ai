...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_ATTACK) then
		if (ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana) then
			...
			if (locationAoE.count >= 2 and GetUnitToLocationDistance(npcBot, locationAoE.targetloc) <= CastRange) then
	...
	then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy) and GetUnitToUnitDistance(npcBot, npcEnemy) < CastRange) then
...
Consider[2] = function()
	...
	local Damage =
		math.min(#searchcreeps, ability:GetSpecialValueInt("max_units")) * ability:GetSpecialValueInt("damage_per_unit")
...
Consider[3] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			...
			then
				...
				if (#enemys + #allys >= 4 and #enemys >= 1 and GetUnitToUnitDistance(npcBot, WeakestEnemy) <= CastRange) then
					if (trees2 ~= nil and #trees2 >= 1) then
						return BOT_ACTION_DESIRE_MODERATE - 0.02, GetTreeLocation(trees2[#trees2])
					else
						return BOT_ACTION_DESIRE_MODERATE - 0.02,
							utility.GetUnitsTowardsLocation(WeakestEnemy, npcBot, Radius / 2)
					end
				end
			end
		end
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		if (npcBot:WasRecentlyDamagedByAnyHero(2)) then
			if (trees ~= nil and #trees >= 1 and #enemys >= 1) then
	...
	if
		(npcBot:GetActiveMode() == BOT_MODE_PUSH_TOWER_TOP or npcBot:GetActiveMode() == BOT_MODE_PUSH_TOWER_MID or
			...
			npcBot:GetActiveMode() == BOT_MODE_DEFEND_TOWER_BOT)
	then
		if (#enemys + #allys >= 4 and #enemys >= 1 and #towers >= 1) then
	...
	then
		...
		if (npcEnemy ~= nil) then
			...
			if (#enemys + #allys >= 4 and #enemys >= 1 and GetUnitToUnitDistance(npcBot, npcEnemy) <= CastRange) then
				if (trees2 ~= nil and #trees2 >= 1) then
					return BOT_ACTION_DESIRE_MODERATE - 0.02, GetTreeLocation(trees2[#trees2])
				else
					return BOT_ACTION_DESIRE_MODERATE - 0.02,
	...
	-- If we're farming and can kill 3+ creeps
	if (npcBot:GetActiveMode() == BOT_MODE_FARM) then
		if (ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana and #creeps >= 8) then
...
Consider[4] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_ATTACK) then
		if (#enemys + #allys + disabledheronum >= 5) then

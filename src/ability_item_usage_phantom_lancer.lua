...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			...
			if CanCast[abilityNumber](WeakestEnemy) and (enemys ~= nil or #enemys2 <= 2) then
				if npcBot:GetMana() > ComboMana * 1.5 then

...
Consider[1] = function()
	...
	--------------------------------------
	--protect myself
	if (npcBot:WasRecentlyDamagedByAnyHero(2) or npcBot:GetActiveMode() == BOT_MODE_RETREAT) then
		return BOT_ACTION_DESIRE_HIGH
	end
...
Consider[4] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_FARM) then
		if (#creeps >= 1) then
			for _, tempcreep in pairs(creeps) do
				if (tempcreep:GetHealth() >= 600 and npcBot:GetMana() > ComboMana and CanCast[abilityNumber](tempcreep)) then

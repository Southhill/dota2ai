...
local CanCast = {
	...
	function(t)
		if
			not AbilityExtensions:AllyCanCast(t) or
				AbilityExtensions:MustBeIllusion(npcBot, t) and not AbilityExtensions:IsTempestDouble(t) or
				not AbilityExtensions:IsHero(t) and not AbilityExtensions:IsLoneDruidBear(t)
		 then
			return false
		end
		if AbilityExtensions:DontInterruptAlly(t) then
			...
			if
				t:HasModifier("modifier_monkey_king_fur_army_soldier_in_position") and AbilityExtensions:GetHealthPercent(t) <= 0.35
			 then
				return true
			end
			return false
		...
		return AbilityExtensions:AllyCanCast(t) and
			(not AbilityExtensions:DontInterruptAlly(t) or
				t:HasModifier("modifier_medusa_stone_gaze") and t:GetActiveMode() == BOT_MODE_RETREAT) and
			not t:IsChanneling()
	end,
	function(t)
...
Consider[1] = function()
	...
	local enemys = utility.GetNearbyVisibleHeroes(npcBot, AttackRange, true, BOT_MODE_NONE)
...
function GetAbilityTarget(npcTarget)
	...
	if (tableNearbyEnemyCreeps ~= nil) then
		for _, c in pairs(tableNearbyEnemyCreeps) do
			if GetUnitToUnitDistance(c, npcTarget) < Radius and CanCast[2](c) then
	...
	if (tableNearbyEnemyHeroes ~= nil) then
		for _, h in pairs(tableNearbyEnemyHeroes) do
			if h:GetUnitName() ~= npcTarget:GetUnitName() and GetUnitToUnitDistance(h, npcTarget) < Radius and CanCast[2](h) then
	...
	if (npcBot:GetActiveMode() == BOT_MODE_FARM) then
		if (#creeps >= 2) then
			if
			 ...
			 then
				if (CanCast[abilityNumber](WeakestCreep)) then
					local desire, target = GetAbilityTarget(WeakestCreep)
					if (desire > 0) then
						return desire, target
					end
...
Consider[3] = function()
	...
	allys =
		...
		function(t)
			return AbilityExtensions:MayNotBeIllusion(npcBot, t) and not AbilityExtensions:CannotBeTargetted(t) and
				not AbilityExtensions:IsInvulnerable(t) and
				not AbilityExtensions:CannotBeKilledNormally(t)
		end
	)
	 ...
	 then
		...
		for _, npcTarget in pairs(allys) do
			...
			if
			 ...
			 then
				if (CanCast[abilityNumber](npcTarget) and not npcTarget:IsChanneling()) then
	...
	if
	 ...
	 then
		for _, npcTarget in pairs(allys) do
			if
				npcTarget:GetHealth() / npcTarget:GetMaxHealth() < (0.35 + 0.4 * ManaPercentage) and
...
Consider[4] = function()
	...
	for _, npcEnemy in pairs(enemys) do
		if (npcEnemy:IsChanneling() and CanCast[abilityNumber](npcEnemy) and #allys < #enemys) then
			return BOT_ACTION_DESIRE_HIGH, npcEnemy
		end
	end

	if (enemys ~= nil and #enemys == 1) then
		return 0
	end
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		...
		for i, npcEnemy in pairs(enemys) do
			...
			if (npcEnemy ~= nil) then
				for j, npcEnemyAttacker in pairs(enemys2) do
					if (npcEnemy ~= npcEnemyAttacker) then
						sumdamage =
							sumdamage + npcEnemyAttacker:GetEstimatedDamageToTarget(true, npcEnemy, Duration, DAMAGE_TYPE_PHYSICAL)
					end
				end
				...
				if (sumdamage / npcEnemy:GetHealth() > BestTargetScore) then
		...
		if (BestTargetScore >= 1.05 and BestTarget ~= nil) then
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		for _, npcEnemy in pairs(enemys) do
			if (npcBot:WasRecentlyDamagedByHero(npcEnemy, 2.0)) then
				if (CanCast[abilityNumber](npcEnemy) and not enemyDisabled(npcEnemy) and #allys <= 1) then
					return BOT_ACTION_DESIRE_HIGH, npcEnemy
				end
			end
		end

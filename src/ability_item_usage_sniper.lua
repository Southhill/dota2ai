...
Consider[1] = function()
	...
	if (desire ~= 0) then
		for _, i in pairs(npcBot.Sniper1) do
			if (DotaTime() >= i.timer + 10) then
				table.remove(npcBot.Sniper1, _)
			end
			if (utility.PointToPointDistance(i.location, templocation)) < Radius * 1.5 or (DotaTime() - i.timer <= 2) then
				desire = 0
			end
		end
	end
...
Consider[5] = function()
	...
	local allys = utility.GetNearbyVisibleHeroes(npcBot, 1200, false, BOT_MODE_NONE)
	local enemys2 = utility.GetNearbyVisibleHeroes(npcBot, 1000, true, BOT_MODE_NONE)
	local enemys3 = GetUnitList(UNIT_LIST_ENEMY_HEROES)
	local enemys = {}
	for i, enemy in pairs(enemys3) do
		if (GetUnitToUnitDistance(npcBot, enemy) < CastRange) then
			...
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if
					(HeroHealth + WeakestEnemy:GetHealthRegen() * CastPoint <=
						WeakestEnemy:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL) and
						GetUnitToUnitDistance(WeakestEnemy, npcBot) > AttackRange and
	...
	if
	 ...
	 then
		if (ManaPercentage > 0.6 or npcBot:GetMana() > ComboMana) then
			if (WeakestEnemy ~= nil and #enemys2 == 0) then
				if (CanCast[abilityNumber](WeakestEnemy)) then
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil and #enemys2 == 0) then
			if

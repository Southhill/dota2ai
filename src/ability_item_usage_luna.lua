...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if ((ManaPercentage > 0.65 and npcBot:GetMana() > ComboMana) and ability:GetLevel() >= 2) then
			if (WeakestEnemy ~= nil) then
				if (CanCast[abilityNumber](WeakestEnemy)) then
...
Consider[4] = function()
	...
	if (npcBot:HasScepter() or npcBot:HasModifier("modifier_item_ultimate_scepter")) then
		local locationAoE = npcBot:FindAoELocation(true, true, npcBot:GetLocation(), CastRange, Radius, 0, 0)
		if (locationAoE.count - #creeps >= 2) then
		...
		if (npcBot:GetActiveMode() == BOT_MODE_ATTACK) then
			...
			for _, npcAlly in pairs(allys) do
				...
				if (#enemys2 + disabledheronum - #creeps >= 2) then

	...
	function(t)
		if npcBot:GetTeam() == t:GetTeam() then
			...
			return AbilityExtensions:NormalCanCast(t, false, DAMAGE_TYPE_MAGICAL) and
				not t:HasModifier("modifier_antimage_counterspell")
		end
	end,
	utility.NCanCast,
	function(t)
...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if (WeakestEnemy:GetHealth() <= WeakestEnemy:GetActualIncomingDamage(GetComboDamage(), DAMAGE_TYPE_ALL)) then
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (WeakestCreep ~= nil and ManaPercentage > 0.45) then
...
local oldConsider2 = function()
	...
	for _, enemy in pairs(enemys) do
		if (enemy:IsChanneling() and CanCast[abilityNumber](enemy)) then
...
Consider[4] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy) and #enemys >= 2) then
				if

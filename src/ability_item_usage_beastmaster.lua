...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (ManaPercentage > 0.6 or npcBot:GetMana() > ComboMana) then
			...
			if (locationAoE.count >= 2 and GetUnitToLocationDistance(npcBot, locationAoE.targetloc) <= CastRange - 100) then
...
Consider[3] = function()
	...
	if (runeDistance[1] < runeDistance[2]) then
		...
		for _, npcEnemy in pairs(enemys) do
			if
				((npcBot:WasRecentlyDamagedByHero(npcEnemy, 2.0) and CanCast[abilityNumber](npcEnemy)) or
					...
					not AbilityExtensions:HasDamageRetargetModifier(npcEnemy)
			 then
				return BOT_ACTION_DESIRE_HIGH, npcEnemy
			end
		end

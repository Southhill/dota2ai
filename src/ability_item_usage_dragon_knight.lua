...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (WeakestCreep ~= nil) then
			if
				((ManaPercentage > 0.7 or npcBot:GetMana() > ComboMana * 1.5) and GetUnitToUnitDistance(npcBot, WeakestCreep) >= 300)
			 then
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if ((ManaPercentage > 0.7 or npcBot:GetMana() > ComboMana * 1.5) and ability:GetLevel() >= 2) then

...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if ((ManaPercentage > 0.6 or npcBot:GetMana() > ComboMana) and ability:GetLevel() >= 2) then
			if (WeakestEnemy ~= nil) then
				if (CanCast[abilityNumber](WeakestEnemy)) then
	...
	if
	 ...
	 then
		...
		if (#creeps >= 3) then
			if (ManaPercentage > 0.5 or npcBot:GetMana() > ComboMana) then
				if (CanCast[abilityNumber](creeps[1])) then
...
Consider[6] = function()
	...
	if (#tableNearbyAttackingAlliedHeroes + #enemys >= 5 and #enemys >= 1) then
...
Consider[4] = function()
	...
	if
		AbilitiesReal[4]:GetCooldown() - AbilitiesReal[4]:GetCooldownTimeRemaining() > 5 or
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		for _, npcEnemy in pairs(enemys) do
			if (npcBot:WasRecentlyDamagedByHero(npcEnemy, 2.0) == false) then
				if (CanCast[abilityNumber](npcEnemy)) then

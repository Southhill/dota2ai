...
Consider[1] = function()
	...
	if
	 ...
	 then
		if (#enemys >= 1) then
			if (ManaPercentage > 0.5 or npcBot:GetMana() > ComboMana and AbilitiesReal[abilityNumber]:GetLevel() >= 1) then
				if (WeakestEnemy ~= nil) then
		...
		if (#creeps >= 3) then
			if (ManaPercentage > 0.5 or npcBot:GetMana() > ComboMana and AbilitiesReal[abilityNumber]:GetLevel() >= 1) then
				if (CanCast[abilityNumber](creeps[1]) and GetUnitToUnitDistance(npcBot, creeps[1]) < CastRange + 75 * #allys) then
...
function GetEarlyLocation(npcEnemy)
	...
	for k, t in pairs(infoTable) do
		...
		if (time1 >= 3.9 and time1 <= 4.1) then
			return location
		end
...
Consider[2] = function()
	...
	if
		(npcBot:GetActiveMode() == BOT_MODE_PUSH_TOWER_TOP or npcBot:GetActiveMode() == BOT_MODE_PUSH_TOWER_MID or
			...
			npcBot:GetActiveMode() == BOT_MODE_DEFEND_TOWER_BOT)
	 then
		if (#enemys >= 1 and #enemys > #allys) then
			for k, npcEnemy in pairs(enemys) do
				if (GetUnitToLocationDistance(npcBot, GetEarlyLocation(npcEnemy)) >= 3000) then
					return BOT_ACTION_DESIRE_HIGH, npcEnemy
				end
			end
		end
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil and #allys >= #enemys) then
			if
				(CanCast[abilityNumber](npcEnemy) and GetUnitToLocationDistance(npcBot, GetEarlyLocation(npcEnemy)) <= 500 and
					GetUnitToUnitDistance(npcBot, npcEnemy) > 700 and
...
Consider[4] = function()
	...
	channelingEnemies =
		...
		channelingEnemies,
		function(t)
			return t:GetCurrentActiveAbility():GetCooldown()
		end
	)
	if
		channelingEnemies ~= nil and
			channelingEnemies[1]:GetCurrentActiveAbility():GetCooldown() >= ability:GetCooldown() * 0.7
	 then
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if CanCast[abilityNumber](npcEnemy) and AbilityExtensions:IsNonIllusionHero(npcEnemy) then

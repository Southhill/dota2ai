...
Consider[1] = function()
	...
	--then
	if ((ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana) and ability:GetLevel() >= 1) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[1](WeakestEnemy)) then
...
Consider[2] = function()
	...
	local npcTarget = npcBot:GetTarget()
	if (npcTarget ~= nil and CanCast[2](npcTarget)) then
		if (GetComboDamage() > npcTarget:GetHealth()) then
	...
	then
		...
		if (npcTarget ~= nil) then
			if (CanCast[2](npcTarget) and GetUnitToUnitDistance(npcBot, npcTarget) < CastRange) then
...
Consider[3] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[3](WeakestEnemy)) then
				if
					(HeroHealth <= WeakestEnemy:GetActualIncomingDamage(GetComboDamage(), DAMAGE_TYPE_MAGICAL) and
						npcBot:GetMana() > ComboMana)
				then
	...
	if (npcTarget ~= nil) then
		if (CanCast[3](npcTarget)) then
			if (GetComboDamage() > npcTarget:GetHealth() and GetUnitToUnitDistance(npcTarget, npcBot) < (CastRange + 200)) then
...
Consider[4] = function()
	...
	-- If a mode has set a target, and we can kill them, do it
	local npcTarget = AbilityExtensions:GetTargetIfGood(npcBot)
	if (npcTarget ~= nil and CanCast[4](npcTarget)) then
		if

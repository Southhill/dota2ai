...
function isNoCreeps(npcBot, lane) --判断兵线位置在不在塔前，不要越塔
	...
	if
		(TowerDistance < GetUnitToUnitDistance(EnemyTower, AllyTower) and
			PointToPointDistance(front, EnemySpawnLocation) < DistanceTowerToEnemyHome - MaxDiveLength)
	 then
		return true
	end
...
function getTargetLocation(npcBot, lane)
	...
	if (TowerDistance < 1000) then
		...
		-- end
		if
			(PointToPointDistance(front, EnemySpawnLocation) < DistanceTowerToEnemyHome - MaxDiveLength and
				npcBot:GetLevel() <= 12)
		 then
...
local function getMyTarget(npcBot, lane, TargetLocation)
	...
	if
		(creeps ~= nil and #creeps >= 1 and (GetUnitToLocationDistance(npcBot, TargetLocation) <= 200 or TowerDistance <= 800))
	 then
...
function UnitPushLaneThink(npcBot, lane)
	...
	elseif npcBot:WasRecentlyDamagedByTower(1) then --if I'm under attck of tower, then try to avoid attack
		if
			not TransferHatred(npcBot) or (enemys ~= nil and #enemys >= 1) or
				AbilityExtensions:Any(
					npcBot:GetNearbyTowers(700, true),
					function(t)
...
function AssembleWithAlly(unit)
	...
	if (TPresult == false) then
		...
		print(
			getCurrentFileName() .. " " .. getShortName(unit) .. "'s enemy is too much, fall back. try tp " .. tostring(TPresult)
		)
	end
end
...
function StepBackCreeps(unit)
	...
	if #friendCreeps == 0 then
		if
			AbilityExtensions:GetHealthPercent(unit) >= 0.75 or
				#friends >= #enemies + 1 and AbilityExtensions:GetHealthPercent(unit) >= 0.6
		 then
			if tower:HasModifier("modifier_fountain_glyph") then
				unit:Action_MoveDirectly(
					AbilityExtensions:GetPointFromLineByDistance(tower:GetLocation(), unit:GetLocation(), tower:GetAttackRange() + 10)
				)
			end
		end
	else

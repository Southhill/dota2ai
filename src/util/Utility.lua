...
-- gxc's code
-- 创建于: 2017/03/16
-- nBehavior = 技能的 TargetTeam, TargetType, TargetFlags 或 Behavior 函数返回值
-- nFlag = 技能目标队伍、目标类型、目标标志或行为位域常量
-- 用于检查技能行为标志位是否包含指定的标志
...
-- 在施法范围内随机找一个落点（用于闪烁技能等），最多尝试10次
function utilityModule.RandomInCastRangePoint(unit, target, CastRange, distance)
	local i = 0
	local l, d
	repeat
		l =
			utilityModule.GetUnitsTowardsLocation(unit, target, GetUnitToUnitDistance(unit, target) / 2) +
...
function utilityModule.GetAlliesNearLocation(loc, dist)
	...
	for _, enID in pairs(GetTeamPlayers(GetTeam())) do
		...
		if enemyInfo ~= nil and enemyInfo["location"] ~= nil then
			if
				IsHeroAlive(enID) and utilityModule.GetDistance(enemyInfo["location"], loc) <= dist and
					(utilityModule.GetDistance(enemyInfo["location"], Vector(0, 0)) > 10) and
					enemyInfo["time_since_seen"] < 10
			 then
...
utilityModule.FillInAbilities = function(npcBot, abilityTable)
	...
	abilityTable.incorrectAbilityLevelUpNumber =
		...
		function(abilityName, index)
			...
			local result =
				index < npcBot:GetLevel() - npcBot:GetAbilityPoints() + 1 and
				(abilityName == AbilityExtensions.IncorrectAbilityName)
			return result
		end
	)
	abilityTable.talentTreeIndex =
		AbilityExtensions:Count(
		abilityTable,
		function(abilityName, index)
...
function utilityModule.IsStuck(npcBot)
	if npcBot.stuckLoc ~= nil and npcBot.stuckTime ~= nil then
		...
		if
			npcBot:GetCurrentActionType() == BOT_ACTION_TYPE_MOVE_TO and attackTarget == nil and EAd > 2200 and TAd > 2200 and
				#Et == 0 and
				#At == 0 and

...
function X.IsRemovedFromSupportPoll(hero)
	return hero == "npc_dota_hero_alchemist" or hero == "npc_dota_hero_naga_siren" or hero == "npc_dota_hero_skeleton_king" or
...
function X.CanBeMidlanerOld(hero)
	...
	return hero == "npc_dota_hero_zuus" or hero == "npc_dota_hero_templar_assassin" or hero == "npc_dota_hero_ember_spirit" or
...
function X.UpdateInvisEnemyStatus(bot)
	if globalEnemyCheck == false then
		...
		for i = 1, #players do
			if X["invisHeroes"][GetSelectedHeroName(players[i])] == 1 then

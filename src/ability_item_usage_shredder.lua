...
Consider[1] = function()
			...
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if
					(HeroHealth <= WeakestEnemy:GetActualIncomingDamage(GetComboDamage(), DAMAGE_TYPE_MAGICAL) and
						...
						GetUnitToUnitDistance(npcBot, WeakestEnemy) < Radius)
				then
	...
	if (ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana) then
		if (npcBot:WasRecentlyDamagedByAnyHero(2) and #enemys >= 2) then
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (WeakestCreep ~= nil) then
			if ((ManaPercentage > 0.65 or npcBot:GetMana() > ComboMana) and GetUnitToUnitDistance(npcBot, WeakestCreep) < Radius) then
...
function GetBestTree(npcBot, enemy, CastRange, hitRadios)
	...
	for _, tree in pairs(trees) do
		...
		if x ~= y then
			...
			if
				d <= hitRadios and mindis > GetUnitToLocationDistance(enemy, x) and
				(GetUnitToLocationDistance(enemy, x) <= GetUnitToLocationDistance(npcBot, x))
			then
				bestTree = x
...
function GetBestRetreatTree(npcBot, CastRange)
	...
	for _, tree in pairs(trees) do
		...
		if
			(not utility.AreTreesBetween(loc, 100)) and GetUnitToLocationDistance(npcBot, loc) > maxdis and
...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		local tableNearbyEnemyHeroes = utility.GetNearbyVisibleHeroes(npcBot, 1200, true, BOT_MODE_NONE)
...
Consider[5] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		for _, npcEnemy in pairs(enemys) do
			if (npcBot:WasRecentlyDamagedByHero(npcEnemy, 2.0)) then
				ultTime = DotaTime()
				return BOT_ACTION_DESIRE_HIGH, npcEnemy:GetExtrapolatedLocation(
					GetUnitToUnitDistance(npcEnemy, npcBot) / Speed + CastPoint
				)
			end
		end
	end
	...
	then
		...
		if (npcTarget ~= nil) then
			...
			if dist < CastRange or npcTarget:GetHealth() / npcTarget:GetMaxHealth() < 0.65 then
				ultTime = DotaTime()
				return BOT_ACTION_DESIRE_HIGH - 0.1, npcBot:GetXUnitsTowardsLocation(
					npcTarget:GetLocation(),
					dist + (dist / Speed + CastPoint) * 350
				)
			end
		end

...
function MinionThink(hMinionUnit)
    if not hMinionUnit:IsNull() and hMinionUnit ~= nil then
        if string.find(hMinionUnit:GetUnitName(), "npc_dota_brewmaster_storm") then
            if (hMinionUnit:IsUsingAbility()) then
                return
            end
...
function ConsiderCY(hMinionUnit)
    ...
    if (#tableNearbyAttackingAlliedHeroes >= 1) then
    ...
    for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
        if (CanCastCYOnTarget(npcEnemy) and not IsDisabled(npcEnemy) and npcEnemy:GetActiveMode() == BOT_MODE_RETREAT) then
            return BOT_ACTION_DESIRE_LOW, npcEnemy
        end
    end
...
function ConsiderCorrosiveHaze(hMinionUnit)
    ...
    if (#tableNearbyAttackingAlliedHeroes >= 1) then
        ...
        local EnemyHeroes = hMinionUnit:GetNearbyHeroes(1000, true, BOT_MODE_NONE)
        for _, npcEnemy in pairs(EnemyHeroes) do
            if (CanCastCYOnTarget(npcEnemy)) then
                local nDamage = npcEnemy:GetEstimatedDamageToTarget(false, hMinionUnit, 3.0, DAMAGE_TYPE_ALL)
                if (nDamage > nMostDangerousDamage) then
                    nMostDangerousDamage = nDamage
                    npcMostDangerousEnemy = npcEnemy
    ...
    for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
        if (CanCastCYOnTarget(npcEnemy) and npcEnemy:GetActiveMode() == BOT_MODE_RETREAT) then
            return BOT_ACTION_DESIRE_LOW, npcEnemy
        end
    end

...
----------------------------------------------------------------------------
--------------------------------------
-- General Initialization 大恶魔TB (恐怖利刃)
--------------------------------------
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy) and GetUnitToUnitDistance(npcBot, npcEnemy) < Radius - 120) then
...
Consider[2] = function()
	...
	if
		(npcBot:GetActiveMode() == BOT_MODE_FARM or npcBot:GetActiveMode() == BOT_MODE_PUSH_TOWER_MID or
	...
	if (#tableNearbyAttackingAlliedHeroes >= 2) then
		...
		for _, npcEnemy in pairs(enemys) do
			if (CanCast[abilityNumber](npcEnemy)) then
				...
				if nEnemyHP > nMostHealth and nEnemyHP / npcEnemy:GetMaxHealth() > 0.6 then

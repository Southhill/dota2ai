...
local GAME_MIDDLE_25_TIME = 25 * 60 -- 游戏中期的25分钟
local GAME_MIDDLE_35_TIME = 35 * 60 -- 游戏中期的35分钟
...
-- 次要操作循环：未实现物品 + 卡住检测
-- 团队级操作（符文、遗迹告警）已移至 team_desires.lua 的 TeamThink()
...
function ability_item_usage_generic.CourierUsageThink()
    -- 驱动当前 Lua 状态的 Timer 模块（帧计数器、节流器、协程）
    -- 每个 hero 脚本有独立 Lua 状态，Timer 非全局单实例
    Timer.TickFromDota()
...
-- 技能加点主函数（供各英雄调用）
function ability_item_usage_generic.AbilityLevelUpThink(AbilityToLevelUp, TalentTree)
...
function ability_item_usage_generic.BuybackUsageThink()
    ...
    -- 米波克隆体不买活
    -- 注意：GetBot() 可能返回米波的分则能成克隆体（引擎为每个克隆体运行独立的脚本实例）
    -- 克隆体只有鞋和 TP，不应执行买活、加点等主身操作
    ...
    -- 35分钟后，复活时间>=30秒则自动买活
    if (DotaTime() > GAME_MIDDLE_35_TIME and CanBuybackUpperRespawnTime(30)) then
        npcBot:ActionImmediate_Buyback()
    end
end
...
function ability_item_usage_generic.InitAbility(Abilities, AbilitiesReal, Talents)
    ...
    npcBot.abilityInited = true -- 每次脚本重载时设置为true
end
        ...
        if (cast.Desire[i] ~= nil and cast.Desire[i] > 0) then
            ...
            if ((cast.Type[i] == nil or cast.Type[i] == "Target") and cast.Target[i] ~= nil and
                ...
                if (cast.Target[i] ~= nil) then
                    ...
                    utility.DebugTalk("尝试使用技能" .. i .. " 欲望：" .. cast.Desire[i])
                end
            else
                utility.DebugTalk("尝试使用技能" .. i .. " 欲望：" .. cast.Desire[i])
            end

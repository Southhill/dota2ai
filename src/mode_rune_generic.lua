...
function GetBotClosestRune()
	...
	for _, r in pairs(ListRune) do
		...
		if
			not IsHumanPlayerNearby(rLoc) and not IsPingedByHumanPlayer(rLoc) and not IsThereMidlaner(rLoc) and
				not IsThereCarry(rLoc) and
				IsTheClosestOne(rLoc)
		 then
...
function IsPingedByHumanPlayer(runeLoc)
	...
	for _, p in pairs(listPings) do
		if
			p ~= nil and not p.normal_ping and GetDistance(p.location, runeLoc) < 1200 and dist2 < 1200 and
				GameTime() - p.time < PingTimeGap
		 then
			return true
		end
	end
...
function IsThereCarry(runeLoc)
	for k, v in pairs(teamPlayers) do
		local member = GetTeamMember(k)
		if
			member ~= nil and not member:IsIllusion() and member:IsAlive() and role.CanBeSafeLaneCarry(member:GetUnitName()) and
...
function IsSuitableToPick()
	...
	if
		(mode == BOT_MODE_RETREAT and bot:GetActiveModeDesire() > BOT_MODE_DESIRE_MODERATE) or mode == BOT_MODE_ATTACK or
			mode == BOT_MODE_DEFEND_ALLY or
			mode == BOT_MODE_DEFEND_TOWER_TOP or
			mode == BOT_MODE_DEFEND_TOWER_MID or
			mode == BOT_MODE_DEFEND_TOWER_BOT or
			(#Enemies >= 1 and IsIBecameTheTarget(Enemies)) or

...
function TurboModeLogic()
	...
	if
		GetHeroPickState() == 55 and
			((humanInRad1Slot == true and IsHumanDonePickingFirstSlot() and DotaTime() > -10 and DotaTime() < -5) or
				(humanInRad1Slot == false and GameTime() > 10 and DotaTime() > -10 and DotaTime() < -5))
	 then
		for i, id in pairs(GetTeamPlayers(GetTeam())) do
			if IsPlayerBot(id) and IsPlayerInHeroSelectionControl(id) and GetSelectedHeroName(id) == "" then
				if debugMode then
...
function AllRandomLogic()
	for i, id in pairs(GetTeamPlayers(GetTeam())) do
		if
			GetHeroPickState() == HEROPICK_STATE_AR_SELECT and IsPlayerInHeroSelectionControl(id) and
		...
		if IsHumansDonePicking() then
			if IsHumanPlayerExist() then
				...
				if selectedHero ~= "" and selectedHero ~= nil then
					for i, id in pairs(GetTeamPlayers(GetTeam())) do
						if
							GetHeroPickState() == HEROPICK_STATE_AP_SELECT and IsPlayerBot(id) and IsPlayerInHeroSelectionControl(id) and
								GetSelectedHeroName(id) == ""
						 then
			...
			for position = 1, 5, 1 do
				if
					(ListContains(hero_pool_position[position], heroName) or
						ListContains(hero_pool_position_unimplemented[position], heroName))
				 then
...
function APLaneAssignment()
	...
	if
		(gamestate == GAME_STATE_HERO_SELECTION or gamestate == GAME_STATE_STRATEGY_TIME or
			gamestate == GAME_STATE_TEAM_SHOWCASE or
			gamestate == GAME_STATE_WAIT_FOR_MAP_TO_LOAD or
			gamestate == GAME_STATE_WAIT_FOR_PLAYERS_TO_LOAD)
	 then
		return
	end
...
function CaptainModeLogic()
	...
	if GetHeroPickState() == HEROPICK_STATE_CM_CAPTAINPICK then
		PickCaptain()
	elseif
		GetHeroPickState() >= HEROPICK_STATE_CM_BAN1 and GetHeroPickState() <= 18 and GetCMPhaseTimeRemaining() <= NeededTime
	 then
		BansHero()
		NeededTime = 0
	elseif
		GetHeroPickState() >= HEROPICK_STATE_CM_SELECT1 and GetHeroPickState() <= HEROPICK_STATE_CM_SELECT10 and

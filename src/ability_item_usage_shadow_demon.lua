...
Consider[1] = function()
	...
	for _, enemy in pairs(enemys) do
		if
			(enemy:IsChanneling() and CanCast[abilityNumber](enemy) and
				not AbilityExtensions:IsOrGoingToBeSeverelyDisabled(enemy))
		 then
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if (CanCast[abilityNumber](WeakestEnemy) and not AbilityExtensions:IsOrGoingToBeSeverelyDisabled(WeakestEnemy)) then
		...
		if (npcTarget ~= nil) then
			if
				(CanCast[abilityNumber](npcTarget) and not AbilityExtensions:IsOrGoingToBeSeverelyDisabled(npcTarget) and
					GetUnitToUnitDistance(npcBot, npcTarget) > 400 and
		...
		if
		 ...
		 then
			return RemapValClamped(
				AbilityExtensions:GetIllusionBattlePower(npcTarget),
				0.1,
				0.4,
...
Consider[2] = function()
	...
	do
		if target and CanCast[2](target) then
			return BOT_ACTION_DESIRE_MODERATE, AbilityExtensions:FindAOELocationAtSingleTarget(
				npcBot,
				target,
				radius,
				castRange,
...
Consider[3] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if
					(ManaPercentage > 0.5 and HeroHealth <= WeakestEnemy:GetActualIncomingDamage(GetComboDamage(), DAMAGE_TYPE_MAGICAL) and
						npcBot:GetMana() > ComboMana and
						GetPoisonCount(WeakestEnemy) < 5)
				 then

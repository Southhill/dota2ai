...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (WeakestCreep ~= nil) then
			if (ManaPercentage > 0.8 and GetUnitToUnitDistance(npcBot, WeakestCreep) >= AttackRange + 100) then
...
Consider[2] = function()
	...
	if
		((npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) or
			(npcBot:GetActiveMode() == BOT_MODE_ATTACK and #nearyByEnemys >= 2))
	 then
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (GetUnitToUnitDistance(npcBot, npcEnemy) < radius + 75 * #allys) then
...
Consider[4] = function()
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy)) and GetUnitToUnitDistance(npcBot, npcEnemy) <= 1800 then
				return BOT_ACTION_DESIRE_MODERATE
			end
		end
	end

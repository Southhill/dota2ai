...
Consider[3] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		local BestTarget
		if (npcBot:GetMaxMana() - npcBot:GetMana() <= DrainMana and #enemys <= #allys) then
			for _, npcEnemy in pairs(enemys) do
				if (CanCast[abilityNumber](npcEnemy) and npcEnemy:GetMana() > 0.5 * DrainMana) then
					BestTarget = npcEnemy
...
Consider[4] = function()
	...
	if (npcTarget ~= nil and CanCast[abilityNumber](npcTarget)) then
		if

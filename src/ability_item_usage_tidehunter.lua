...
function Consider1New()
	...
	if
	 ...
	 then
		...
		if (locationAoE.count >= 4 and ManaPercentage > 0.3) then
...
Consider[3] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_FARM or npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (#creeps >= 2 and WeakestCreep ~= nil) then -- WeakestCreep can be nil on line 378, although I don't know why
			if
...
Consider[4] = function()
	...
	-- Check for a channeling enemy
	for _, npcEnemy in pairs(enemys) do
		if (npcEnemy:IsChanneling() and CanCast[abilityNumber](npcEnemy) and #enemys >= 2) then
			return BOT_ACTION_DESIRE_HIGH, npcEnemy
		end
	end

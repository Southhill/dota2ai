...
local abilityIndex = Func.ReverseTable(abilityName)
			...
			if (CanCast[abilityNumber](WeakestEnemy)) then
				...
				then
					return BOT_ACTION_DESIRE_HIGH, npcBot:GetXUnitsTowardsLocation(
						WeakestEnemy:GetExtrapolatedLocation(CastPoint),
						300
					)
				end
			end
		...
		if (npcEnemy ~= nil) then
			if (CanCast[abilityNumber](npcEnemy)) then
				return BOT_ACTION_DESIRE_HIGH,
					npcBot:GetXUnitsTowardsLocation(npcEnemy:GetExtrapolatedLocation(CastPoint), 300)
			end
		end
	end
...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if ((ManaPercentage > 0.7 or npcBot:GetMana() > ComboMana) and ability:GetLevel() >= 2) then
			if (WeakestEnemy ~= nil) then
				if (CanCast[abilityNumber](WeakestEnemy)) then
...
Consider[3] = function()
	...
	for _, myFriend in pairs(allys) do
		...
		if (CanCast[abilityNumber](myFriend) and tempMana < ManaPercentage) then
			bestTarget = myFriend
			minMana = tempMana
		end
	end

	if (bestTarget ~= nil and minMana <= 0.8) then
...
Consider[abilityIndex.keeper_of_the_light_blind_light] = function()
	...
	local allys = utility.GetNearbyVisibleHeroes(npcBot, 1200, false, BOT_MODE_NONE)
	local enemys = utility.GetNearbyVisibleHeroes(npcBot, Min(CastRange + 300, 1600), true, BOT_MODE_NONE)
	local WeakestEnemy, HeroHealth = utility.GetWeakestUnit(enemys)
	...
	if (#tableNearbyAttackingAlliedHeroes >= 2) then
		...
		if (npcEnemy ~= nil and enemys ~= nil and #enemys >= 2) then
...
Consider[6] = function()
	...
	local enemys2 = utility.GetNearbyVisibleHeroes(npcBot, Radius, false, BOT_MODE_NONE)
	local WeakestEnemy, HeroHealth = utility.GetWeakestUnit(enemys)
...
function AbilityUsageThink()
	...
	if
		npcBot:IsCastingAbility() or npcBot:IsUsingAbility() or npcBot:IsInvulnerable() or npcBot:IsChanneling() or
		npcBot:IsSilenced() or
		npcBot:HasModifier("modifier_doom_bringer_doom") or
		npcBot:NumQueuedActions() > 0
	then
		return
	end

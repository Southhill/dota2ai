...
Consider[2] = function()
	...
	if (ManaPercentage >= 0.3) then
		for k, creep in pairs(creepsNeutral) do
			if (IsGoodNeutralCreeps(creep) or (creep:IsAncientCreep() and AbilityExtensions:HasScepter(npcBot))) then
...
Consider[4] = function()
	...
	local function IsSeverelyDamaged(npc)
		return (AbilityExtensions:GetHealthPercent(npc) <= 0.3 or npc:GetHealth() <= 400) and
			AbilityExtensions:IsSeverelyDisabled(npc) and

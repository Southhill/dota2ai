...
Consider[1] = function()
	...
	if (WeakestCreep ~= nil) then
		if ((ManaPercentage > 0.5 and npcBot:GetMana() > ComboMana) and GetUnitToUnitDistance(npcBot, WeakestCreep) >= 300) then
...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[4](WeakestEnemy)) then
				if
					(HeroHealth <= WeakestEnemy:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL) or
						(HeroHealth <= WeakestEnemy:GetActualIncomingDamage(GetComboDamage(), DAMAGE_TYPE_MAGICAL) and
							npcBot:GetMana() > ComboMana))
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (WeakestEnemy ~= nil and CanCast[4](WeakestEnemy)) then
			if (ManaPercentage > 0.66 or npcBot:GetMana() > ComboMana) then
...
local function TryUseShardLagunaAt(location, possibleEnemies)
	...
	return AbilityExtensions:Count(
		possibleEnemies,
		function(t)

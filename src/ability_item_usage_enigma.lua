...
function CanCast2(npcTarget)
	if (utility.NCanCast(npcTarget) and npcTarget:GetLevel() <= 5 and npcTarget:IsAncientCreep() == false) then
		return true
	end
	return false
end
...
Consider[4] = function()
	...
	local allys =
		...
		AbilityExtensions:GetNearbyNonIllusionHeroes(npcBot),
		function(t)
	...
	channelingEnemies =
		...
		channelingEnemies,
		function(t)
			return AbilityExtensions:IsChannelingAbility(t) and not t:IsInvulnerable() and t:CanBeSeen()
		end
	)
	channelingEnemies =
		...
		channelingEnemies,
		function(t)
			return {t, t:GetCurrentActiveAbility()}
		end
	)
	channelingEnemies =
		AbilityExtensions:Map(
		channelingEnemies,
		function(t)
			return {t[1], t[2] and t[2]:GetCooldown() or 25}
		end
	)
	channelingEnemies =

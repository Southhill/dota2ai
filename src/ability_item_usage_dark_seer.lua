...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (ManaPercentage > 0.55 and npcBot:GetMana() > ComboMana) then
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (GetUnitToUnitDistance(npcBot, npcEnemy) < CastRange + 75 * #allys) then
				for _, myFriend in pairs(allys) do
					if (not myFriend:HasModifier("modifier_dark_seer_ion_shell") and myFriend:GetAttackRange() < 300) then
...
Consider[3] = function()
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (GetUnitToUnitDistance(npcBot, npcEnemy) < 1600) then

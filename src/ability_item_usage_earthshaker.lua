...
Consider[1] = function()
	...
	for _, npcEnemy in pairs(enemys) do
		if (npcEnemy:IsChanneling() and CanCast[1](npcEnemy)) then
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (ManaPercentage > 0.75 or npcBot:GetMana() > ComboMana) then
			local locationAoE = npcBot:FindAoELocation(true, true, npcBot:GetLocation(), CastRange, Radius, CastPoint, 0)
			if (locationAoE.count >= 2) then
				return BOT_ACTION_DESIRE_MODERATE, locationAoE.targetloc
			end

			if (WeakestEnemy ~= nil and CanCast[abilityNumber](WeakestEnemy) and not enemyDisabled(WeakestEnemy)) then
...
Consider[2] = function()
	...
	if AbilityExtensions:HasScepter(npcBot) then
		...
		if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
			if (WeakestCreep ~= nil and ManaPercentage > 0.4 or npcBot:GetMana() > ComboMana) then
				if (CreepHealth <= 3 * npcBot:GetAttackDamage() and GetUnitToUnitDistance(npcBot, WeakestCreep) <= 300) then
...
function GetUltDamage(npcTarget)
	...
	local Damage =
		ability:GetSpecialValueInt("AbilityDamage") + count * ability:GetSpecialValueInt("echo_slam_echo_damage")
	return Damage
end
...
Consider[4] = function()
	...
	local checkIfThereAreManyEnemies =
	...
	if
	 ...
	 then
		...
		if (blink ~= nil and blink:IsFullyCastable()) then
			...
			if (locationAoE.count + locationAoE2.count >= 6) then
			...
			if (WeakestEnemy ~= nil) then
				if (CanCast[abilityNumber](WeakestEnemy)) then
					if
						(#enemys > 1 or not WeakestEnemy:WasRecentlyDamagedByAnyHero(1.5)) and
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if
					(#enemys > 1 or not WeakestEnemy:WasRecentlyDamagedByAnyHero(1.5)) and
						(HeroHealth <= WeakestEnemy:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL))
				 then
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil and #enemys >= 2) then
			if (CanCast[abilityNumber](npcEnemy) and GetUnitToUnitDistance(npcBot, npcEnemy) <= Radius) then
				return BOT_ACTION_DESIRE_HIGH
			end
		end

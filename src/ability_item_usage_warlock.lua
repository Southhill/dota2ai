...
local CanCast = {
	utility.NCanCast,
	function(t)
		return t:GetTeam() == npcBot:GetTeam() and AbilityExtensions:AllyCanCast(t) and
			not t:HasModifier("modifier_ice_blast") or
...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		if (HealthPercentage <= 0.5 and npcBot:WasRecentlyDamagedByAnyHero(2.0)) then
	...
	if
	 ...
	 then
		...
		for _, npcTarget in pairs(allys) do
			...
			if (npcTarget:GetHealth() / npcTarget:GetMaxHealth() < healingFactor and npcTarget:WasRecentlyDamagedByAnyHero(2.0)) then
...
Consider[4] = function()
	...
	for _, npcEnemy in pairs(enemys) do
		if npcEnemy:IsChanneling() and not npcEnemy:IsInvulnerable() then

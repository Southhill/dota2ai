...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() ~= BOT_MODE_RETREAT) then
		if (WeakestEnemy ~= nil) then
			if (npcBot:GetMana() > ComboMana and GetUnitToUnitDistance(npcBot, WeakestEnemy) < AttackRange + 200) then
	...
	--------------------------------------
	--protect myself
	if (ManaPercentage > 0.25 or npcBot:GetMana() > ComboMana) then
	...
	if (npcBot:GetActiveMode() == BOT_MODE_LANING) then
		if (ManaPercentage > 0.45 or npcBot:GetMana() > ComboMana) then
			if (WeakestEnemy ~= nil) then
				if (GetUnitToUnitDistance(npcBot, WeakestEnemy) < AttackRange + 200) then
					return BOT_ACTION_DESIRE_LOW
				end
			end
		end
...
Consider[2] = function()
	...
	if
	 ...
	 then
		...
		if (npcEnemy ~= nil) then
			if (GetUnitToUnitDistance(npcBot, npcEnemy) < AttackRange - 50) then
				return BOT_ACTION_DESIRE_MODERATE
			end
		end
	end
...
Consider[6] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		local tableNearbyEnemyHeroes = utility.GetNearbyVisibleHeroes(npcBot, 600, true, BOT_MODE_NONE)
		for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
			if (npcBot:WasRecentlyDamagedByHero(npcEnemy, 2.0) and CanCast[abilityNumber](npcEnemy)) then

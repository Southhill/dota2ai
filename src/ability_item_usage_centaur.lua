...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_FARM) then
		if (#creeps >= 2) then
			if
				CreepHealth <= WeakestCreep:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL) and
				ManaPercentage >= 0.8 + ability:GetManaCost() and
				#enemys == 0
			then
				return BOT_ACTION_DESIRE_LOW
				--,WeakestCreep
...
Consider[2] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_FARM) then
		if (#creeps >= 3) then
			if
				(CreepHealth <= WeakestCreep:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL) and
					HealthPercentage >= 0.4 + selfDamage)
			then
...
Consider[3] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_ROSHAN) then
		...
		if (utility.IsRoshan(npcTarget) and GetUnitToUnitDistance(npcBot, npcTarget) < nRadius) then
			return BOT_ACTION_DESIRE_LOW
		end
	end
	...
	then
		...
		if (CanCast[abilityNumber](npcTarget) and GetUnitToUnitDistance(npcBot, npcTarget) < nRadius) then
			return BOT_ACTION_DESIRE_HIGH
		end
	end
	return BOT_ACTION_DESIRE_NONE
...
Consider[4] = function()
			...
			if (CanCast[abilityNumber](WeakestEnemy)) then
				if
					((HeroHealth <= WeakestEnemy:GetActualIncomingDamage(GetComboDamage(), DAMAGE_TYPE_MAGICAL) and
						...
						GetUnitToUnitDistance(npcBot, WeakestEnemy) >= 600)
				then
					return BOT_ACTION_DESIRE_HIGH
				end
			end
	...
	then
		if (#allys >= 2) then
			...
			if (npcEnemy ~= nil) then
				if (CanCast[abilityNumber](npcEnemy) and GetUnitToUnitDistance(npcBot, npcEnemy) >= 600) then
					return BOT_ACTION_DESIRE_MODERATE
				end
			end
		end

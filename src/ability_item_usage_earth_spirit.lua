	 ...
	 then
		...
		if (npcTarget ~= nil) then
			if (CanCast[abilityNumber](npcTarget)) then
				if nStone >= 1 and (GetUnitToUnitDistance(npcTarget, npcBot) <= nStoneCR + 200) then
...
Consider[6] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
			if (npcBot:WasRecentlyDamagedByHero(npcEnemy, 1.0) and CanCast[abilityNumber](npcEnemy)) then
...
function AbilityUsageThink()
	...
	if (npcBot:IsUsingAbility() or npcBot:IsChanneling() or npcBot:IsSilenced() or npcBot:NumQueuedActions() > 0) then
		return
	end
	...
	if (cast.Desire[6] > 0) then
		npcBot:Action_UseAbility(AbilitiesReal[6])
		return
	end

	if (cast.Desire[5] > 0) then
	...
	if cast.Desire[4] > 0 and AbilitiesReal[4]:IsFullyCastable() and not npcBot:IsChanneling() and not npcBot:IsSilenced() then

...
Consider[1] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) then
		local tableNearbyEnemyHeroes = utility.GetNearbyVisibleHeroes(npcBot, 150, true, BOT_MODE_NONE)
	...
	-- If we're low hp
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT) then
		local tableNearbyEnemyHeroes = utility.GetNearbyVisibleHeroes(npcBot, 200, true, BOT_MODE_NONE)
		if AbilityExtensions:MayNotBeSeen(npcBot) then
			return UseAtTarget(BOT_ACTION_DESIRE_VERYHIGH, npcBot)
		end
	end
